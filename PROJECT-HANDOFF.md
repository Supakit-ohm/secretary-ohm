...
อัปเดตล่าสุด: 2026-09-16 (ข้อ 53 = ทำเป้าสัดส่วนพอร์ต + กราฟมูลค่าพอร์ตตามเวลา ในหน้า Investments (งานที่ 3-4 จาก 4 เรื่องค้างของ Investments — ปิดครบทั้ง 4 เรื่องในเซสชันเดียวตามที่ ohm ขอ จะ push รวมทีเดียว) · **เป้าสัดส่วนพอร์ต**: การ์ดใหม่ต่อจาก Allocation ตั้ง % สูงสุดต่อพอร์ตรวมได้ต่อประเภท (ครบ 7 ประเภท ไม่ใช่แค่ crypto) เทียบกับสัดส่วนจริงตอนนี้ เกินเป้าจะขึ้นสีส้ม+แบนเนอร์เตือน เก็บที่ field ใหม่ `finance.investmentTargets={"<type>":"maxPct"}` ไม่แตะ `investmentValues` เดิม · **กราฟมูลค่าพอร์ตตามเวลา**: การ์ดใหม่ (db-span4) ใช้ Recharts `AreaChart` (ธีมเดียวกับกราฟความคืบหน้า Tracker ที่ยังใช้ Recharts อยู่) — **เบี่ยงจากแผนเดิมในข้อ 4.4 โดยตั้งใจ**: ไม่ได้เปลี่ยน `investmentValues[type]` เป็น history array ทับของเดิม (เสี่ยง migration ตามที่เตือนไว้) แทนที่ด้วย field ใหม่แยกต่างหาก `finance.portfolioSnapshots=[{id,date,value}]` ปลอดภัยกว่าเพราะไม่กระทบโค้ดเดิมที่อ่าน `investmentValues` เลย · บันทึกด้วยมือทีละจุดผ่านปุ่ม + บนหัวการ์ด (วันเดียวกันกดซ้ำจะทับค่าเดิม ไม่สร้างซ้ำ) + ปุ่มลบจุดล่าสุดเผื่อกดพลาด · **ข้อจำกัด: ไม่มีข้อมูลย้อนหลังให้ backfill** (ของเดิมเก็บแค่ค่าล่าสุดทับทุกครั้ง) กราฟจะค่อยๆ มีจุดตามที่ ohm กดบันทึกไปเรื่อยๆ นับจากนี้ · ตรวจ syntax ผ่าน Babel — ยังไม่ได้รัน `smoke-test.js` เต็มรูปแบบ (เหตุผลเดียวกับข้อ 51-52) · commit แล้ว (`a58c875`) **ohm ยังต้อง push เอง** (รวมกับข้อ 51-52 ที่ยังไม่ push เหมือนกัน — 6 commit ทั้งหมดตั้งแต่ f046516) · **Investments รอบใหญ่ (เริ่มข้อ 15, 2026-08-23) ปิดครบ 4 เรื่องที่ค้างไว้แล้ว** ดูสรุปที่ย่อหน้า "เสร็จแล้ว" ด้านล่าง · ก่อนหน้า 2026-09-16: ข้อ 52 = ทำหน้าประวัติ DCA ต่อเหรียญในหน้า Investments (งานที่ 2 จาก 4 เรื่องค้างของ Investments ทำต่อจากข้อ 51 ในเซสชันเดียวกันตามที่ ohm ขอ จะ push รวมทีเดียว) · `BuyHistoryModal` ใหม่ (วางก่อน `AddCoinModal`) แสดง `h.buys[]` เรียงล่าสุดก่อน ต่อวันที่/จำนวนหน่วย/ยอดจ่าย(สกุลเดิม)/ราคาเฉลี่ยต่อหน่วย + สรุปรวม เปิดผ่านปุ่มไอคอนนาฬิกาบนการ์ดเหรียญ (โชว์เฉพาะเหรียญที่มีงวดซื้อแล้ว) · เพิ่ม **ลบงวดซื้อทีละรายการ** (`deleteBuy`) ด้วย — คำนวณย้อนกลับ "ก้อนแรก" ด้วยสูตรเดียวกับ XIRR (`holdingCostTHB` ลบผลรวม `buys[].totalTHB`) แล้วรวมกับงวดที่เหลือหลังลบใหม่ ได้ `quantity`/`avgCost` ถูกต้องเสมอ · state เก็บแค่ `historyForId` ไม่เก็บ holding object เพื่อให้ modal อ่านข้อมูลสดจาก `holdings` เสมอ (กัน stale state ตอนลบ) · ตรวจ syntax ผ่าน Babel — ยังไม่ได้รัน `smoke-test.js` เต็มรูปแบบ (เหตุผลเดียวกับข้อ 51) · commit แล้ว (`feb0276`) **ohm ยังต้อง push เอง** (รวมกับข้อ 51 ที่ยังไม่ push เหมือนกัน) · งานที่เหลือใน Investments (ยังไม่เริ่ม): ตั้งเป้าสัดส่วนพอร์ต, กราฟมูลค่าพอร์ตตามเวลา (ดูข้อ 4.4 — ใหญ่สุด ต้อง migration) · ก่อนหน้า 2026-09-16: ข้อ 51 = ทำ XIRR/ผลตอบแทน %ต่อปี ในหน้า Investments (งานแรกจาก 4 เรื่องค้างของ Investments — ohm เลือกเริ่มจาก XIRR ก่อนจาก 4 ตัวเลือกที่เสนอผ่าน AskUserQuestion) ใช้ข้อมูลที่เก็บไว้แล้ว `h.buys[]`+`firstHeldDate` (ข้อ 16 เดิม) ไม่ต้อง migration โครงข้อมูลเลย · helper ใหม่ต่อท้าย `holdingValueTHB`: `holdingCashflows` (ประกอบกระแสเงินสดจากก้อนแรก(ต้นทุนรวมปัจจุบัน−ผลรวม buys[].totalTHB)+ทุกงวดใน buys[]+มูลค่าปัจจุบันวันนี้), `xirr` (bisection หา r ที่ NPV=0 — เลือก bisection แทน Newton เพราะกระแสเงินสดสลับเครื่องหมายครั้งเดียว ลู่เข้าเสมอ), `holdingXirrPct` · `coinStat` เพิ่ม field `xirrPct` · โชว์ "ผลตอบแทนเฉลี่ย...ต่อปี" ใต้แถบ pnl ในการ์ดแต่ละเหรียญ + แถวรวมในการ์ดสรุปพอร์ตคริปโต (รวมกระแสเงินสดทุกเหรียญเป็น XIRR เดียวของทั้งพอร์ต ไม่ใช่ค่าเฉลี่ยถ่วงน้ำหนักธรรมดา) · เหรียญที่ไม่มี `firstHeldDate` และไม่เคยซื้อเพิ่มผ่าน `buys[]` จะไม่โชว์ค่านี้ (ข้อมูลไม่พอคำนวณ ไม่ error) · ตรวจ syntax ผ่าน Babel (`@babel/core` transformSync OK) — **ยังไม่ได้รัน `smoke-test.js` เต็มรูปแบบ** (โฟลเดอร์นี้ไม่มี node_modules/playwright ติดตั้งไว้ ต้อง `npm i` ใหม่ถ้าจะรัน) · commit แล้ว (`4f1cb7e`) **ohm ยังต้อง push เอง** · งานที่เหลือใน Investments (ยังไม่เริ่ม): หน้าประวัติ DCA ต่อเหรียญ, ตั้งเป้าสัดส่วนพอร์ต, กราฟมูลค่าพอร์ตตามเวลา (ดูข้อ 4.4 — ใหญ่สุด ต้อง migration) · ก่อนหน้า 2026-09-16: ข้อ 50 = ทำ 3 ไอเดียเสริมที่ค้างมาตั้งแต่ข้อ 41 ให้ครบทีเดียว — ohm เลือก**ทั้ง 3 อย่าง**ผ่าน AskUserQuestion แบบ multi-select: **คำคม, ตัวกรอง/แท็ก, สถิติการอ่าน/ดู** — ทำคู่ขนานทั้ง Book Queue และ Playback (สถาปัตยกรรมแยกกันเหมือนเดิมตามข้อ 46) · เพิ่ม field ใหม่บน record เดิม ไม่สร้างเอนทิตีแยก: `tags:[]` (array string) + `quotes:[{id,text,note,date}]` บนทั้งหนังสือ (`bookQueue`) และรีวิว (`mediaReviews`) + `startedDate`/`finishedDate` บนหนังสือ (auto-set ตอนกด "เริ่มอ่าน"/"อ่านจบแล้ว" ผ่าน `advance()` แต่ไม่ทับถ้ามีค่าอยู่แล้ว กันเผลอรีเซ็ตวันที่ตอนแก้มือ) · สร้าง 2 component ใช้ร่วมกัน: `TagsField` (input คั่นจุลภาค + ปุ่มชิปแท็กแนะนำจากแท็กที่มีอยู่แล้วในระบบ กดเติมได้เลย) กับ `QuotesField` (การ์ดคำคมทีละอัน + ฟอร์มเพิ่มคำคม/หมายเหตุ) วางไว้ครั้งเดียวก่อน `BookFormModal` ใช้ซ้ำใน `PlaybackFormModal` · ตัวกรองเป็นชิปกดเลือกได้หลายอัน (`.tag-filter-chip`) แบบ OR อยู่เหนือชั้นวาง "รายการหนังสือทั้งหมด"/"รายการรีวิวทั้งหมด" เท่านั้น — **ตั้งใจไม่ให้กระทบ**การ์ดสถานะ "กำลังอ่าน"/"ต้องอ่าน" หรือตัวเลขสรุปรวม/สถิติ (ยังนับจากข้อมูลทั้งหมดเสมอ) ถ้า ohm อยากให้กรองกระทบส่วนอื่นด้วยบอกได้ · Playback เพิ่มตัวกรองประเภท (ทั้งหมด/วิดีโอ/พอดแคสต์) เป็นชิปแบบเดียวกัน · เพิ่มการ์ดใหม่ 2 ใบต่อหน้า: "สถิติการอ่าน"/"สถิติการดู/ฟัง" (จบเดือนนี้-ปีนี้/รีวิวเดือนนี้-ปีนี้ + คะแนนเฉลี่ย + ช่องที่รีวิวเยอะสุดสำหรับ Playback) กับ "คลังคำคม" (`DashCollapse` ปิดไว้ก่อน รวมคำคมทุกเล่ม/ทุกรีวิวเรียงตามวันที่ล่าสุด ลบทีละอันได้) · เพิ่มข้อมูลตัวอย่าง tags/quotes/วันที่ ใน `DEFAULT_DATA` ให้เห็นผลทันทีตอนเปิดแอปใหม่ · แก้บั๊กระหว่างเทสต์ 2 จุด (ไม่กระทบผู้ใช้จริง ไม่เคยมีปัญหานี้มาก่อนแต่เจอตอนเขียนเทสต์ใหม่): (1) การ์ดสถิติใหม่ใช้คลาส `.db-bignum` ซ้ำกับตัวเลขสรุปบน hero ทำให้ query เดิมในสโมกเทสต์นับตัวเลขผิด ต้องสโคปเพิ่ม `.db-hero` (2) หนังสือที่ไม่มีปกจะมี `.book-shelf-fallback` render ชื่อเล่มซ้ำอีกจุดในกล่องปก ทำให้ query ชื่อเล่มนับซ้ำ 2 เท่า ต้องสโคปไปที่ `.book-shelf-info .db-tname` โดยตรง · smoke test เพิ่ม 15 เช็คใหม่ (8 Book Queue + 7 Playback) ผ่าน **208/208 · NO RUNTIME ERRORS** · **ohm ยังต้อง push เอง** (รวมกับข้อ 48-49 ที่ยังไม่เคย push เหมือนกัน) · ก่อนหน้า 2026-09-16: ข้อ 49 = ช่อง/เพลย์ลิสต์ใน Playback เปลี่ยนจากพิมพ์อิสระ (input+datalist) เป็น `<select id="pb-channel-select">` บังคับเลือกจากชื่อที่มีอยู่แล้ว กันพิมพ์เพี้ยนแล้วแยกกลุ่มโดยไม่ตั้งใจ (ohm เลือกจากเมนู "งานแนะนำต่อ" ที่เสนอ 4 ตัวเลือก) + ตัวเลือก "+ สร้างช่องใหม่..." สลับไปโชว์ช่องพิมพ์ข้อความชั่วคราว มีปุ่มยกเลิกกลับ dropdown · data model ไม่เปลี่ยน (`channel` ยังเป็น string เดิม) · เพิ่ม id `#pb-type-select` ให้ dropdown ประเภทด้วย (กัน query ปนกับ dropdown ช่องใหม่ตอนเทสต์) · smoke test เพิ่ม 4 เช็คใหม่ + แก้ helper เดิมให้เลือกจาก dropdown แทนพิมพ์ ผ่าน **192/192 · NO RUNTIME ERRORS** · **ohm ยังต้อง push เอง** (รวมกับข้อ 48 ที่ยังไม่เคย push เหมือนกัน) · ก่อนหน้า 2026-09-16: ข้อ 48 = Playback เป็นชั้นวาง (shelf) + จัดกลุ่มการ์ดโฟลเดอร์ตามช่อง/เพลย์ลิสต์ — ohm ขอให้ทำเหมือน Book Queue shelf และรวมรายการจากช่องเดียวกันได้ · ตัดสินใจ: field ข้อความล้วน `channel` (ไม่บังคับ ไม่ใช่เอนทิตี playlist แยก) + autocomplete, เปิดโฟลเดอร์แบบขยายในหน้าเดิม (`.pb-folder-panel` ต่อท้ายชั้นวาง ไม่ใช่ modal), ปกโฟลเดอร์ใช้ตอนล่าสุดอัตโนมัติ (ohm เลือก recommended ทั้ง 3 ข้อผ่าน AskUserQuestion) · เปลี่ยน `PlaybackTab` จาก list แบน `db-row` เป็นชั้นวาง `.pb-shelf` (grid ปก 1:1) มีการ์ดโฟลเดอร์ (badge จำนวนตอน+เฉลี่ยคะแนน กดขยายได้ มีปุ่ม "เพิ่มตอนในช่องนี้" prefill ช่อง) · แก้บั๊กระหว่างพัฒนา 2 จุด: (1) `PlaybackFormModal` เปลี่ยนจากตัดสินโหมดด้วย `initial` truthy เป็นรับ prop `mode` ตรงๆ เพราะพังตอนส่ง initial แบบ partial ตอน prefill ช่อง (2) ปุ่ม "เพิ่มข้อมูล" หลักเดิม `onClick={openAdd}` ส่ง event เข้าไปตรงๆ ทำให้หลุดเป็นค่า channel ("[object Object]") ต้องห่อ `()=>openAdd()` · smoke test เพิ่ม 6 เช็คใหม่ + แก้ 2 เช็คเดิมให้ตรงโครง DOM ใหม่ ผ่าน **187/187 · NO RUNTIME ERRORS** · **ohm ยังต้อง push เอง** (รวมกับข้อ 47 ที่ยังไม่เคย push เหมือนกัน — session ก่อนหน้า remote-devices หลุดกลางทางตอนจะเขียนไฟล์ item-47 กลับเครื่อง ต้องส่งผ่านแชทแทนแล้วค่อยเขียนกลับตอนต่อกันได้ในภายหลัง) · ก่อนหน้า 2026-09-14: ข้อ 47 = เพิ่มรูปปกวิดีโอ/พอดแคสต์ในแท็บ Playback — อัปโหลดเองอย่างเดียว (ไม่มี auto-fetch เพราะไม่มี API ฟรีที่เชื่อถือได้) ใช้โค้ดย่อรูปด้วย canvas จาก BookFormModal เดิมซ้ำใน `PlaybackFormModal` (300px, JPEG q.82, data URI) preview 64×64 สี่เหลี่ยมจัตุรัส (คลาสใหม่ `.pb-cover-picker-preview`) + thumbnail 40×40 ในลิสต์ (`.pb-thumb`) + ไอคอน fallback ตามประเภท (VideoIcon/PodcastIcon) · smoke test เพิ่ม 3 เช็ค ผ่าน **181/181 · NO RUNTIME ERRORS** · **ohm ยังต้อง push เอง** (รวมกับข้อ 46 ที่ยังไม่เคย push เหมือนกัน) · ก่อนหน้า 2026-09-14: ข้อ 46 = ทบทวนข้อ 45 ใหม่ตามฟีดแบ็ก ohm — **เลิกเมนูบนแยก** (เมนูเยอะเกินไปแล้ว) ย้ายเป็น**แท็บย่อยในหน้า Books**แทน (แพทเทิร์นเดียวกับ FinancePage) ชื่อแท็บ **"Playback"** (ohm เลือกจาก 4 ตัวเลือกที่เสนอผ่าน AskUserQuestion) + แยกรีวิวหนังสือออกจาก video/podcast: **รีวิวหนังสือย้ายไปติดกับ record หนังสือใน bookQueue ตรงๆ** (เพิ่ม field rating/reviewText ใน BookFormModal โชว์บนชั้นหนังสือ) ส่วนแท็บ Playback เหลือแค่วิดีโอ/พอดแคสต์ 2 ประเภท (ไม่มีหนังสือ) · ลบเมนู "Media" ที่เพิ่งทำในข้อ 45 ออกทั้งหมด (PAGE_TITLES/PageNav/routing/ค้นหา กลับเป็น 7 หน้าเดิม) · smoke test อัปเดตครบ ผ่าน **178/178 · NO RUNTIME ERRORS** · session นี้ device_bash เสีย (Windows update บล็อก mount) ใช้ stage/commit ไฟล์แทนตลอด · **ohm ยังต้อง push เอง** (ข้อ 45 เดิมไม่เคย push จริง ถูกแทนที่ในข้อ 46 นี้ก่อน) · ก่อนหน้า 2026-09-14: ข้อ 44 = ข้อ 43 push แล้วปกขึ้นจริง (Atomic Habits) แต่หนังสือแปลไทย/ญี่ปุ่นไม่มีในฐานข้อมูล Open Library เลยไม่มีปก (ข้อจำกัดของ API ไม่ใช่บั๊ก) — ohm ถามควรอัปโหลดเองทั้งหมดไหม/ข้อเสีย API คืออะไร ตอบไปว่าแนะนำ**ผสมกัน** ohm เลือกทางนี้ → เพิ่ม**อัปโหลดปกเอง**ใน `BookFormModal` (ย่อรูปด้วย canvas เหลือกว้างสุด 300px แล้วฝังเป็น data URI ใน `coverUrl` เดิม ไม่ต้องสร้างระบบไฟล์แยก) + ปุ่ม "กลับไปดึงอัตโนมัติ" สลับกลับได้ + field `coverManual` กัน auto-fetch ไปทับเล่มที่อัปโหลดเอง · smoke test เพิ่ม 3 เช็ค ผ่าน 166/166 · **ohm ยังต้อง push เอง** · ก่อนหน้า 2026-09-14: ข้อ 43 = ยังไม่ขึ้นหลังข้อ 42 → ตรวจสดอีกรอบเจอว่า Google Books ไม่มี key โดน 429 ค้างยาวจริงบน network ของ ohm (ไม่ใช่ burst ชั่วคราว) → **เปลี่ยนแหล่งปกทั้งหมดเป็น Open Library** (`openlibrary.org/search.json`+`covers.openlibrary.org`) ทดสอบสดจาก network เดียวกันก่อน push รอบนี้ได้ผลจริงทันทีทั้ง 2 เล่ม (status 200, ได้ cover_i จริง) กลไก retry/backoff/ปุ่มลองใหม่จากข้อ 42 ใช้ต่อได้หมดไม่ต้องแก้ · smoke test ผ่าน 163/163 · **ohm ยังต้อง push แล้วเช็คอีกที** (แต่รอบนี้ทดสอบยืนยันจริงแล้วก่อน push ไม่ใช่แค่ทฤษฎี) · ก่อนหน้า 2026-09-14: ข้อ 42 = แก้บั๊กด่วน: ปกหนังสือไม่ขึ้นเลยสักเล่มหลังข้อ 41 — สาเหตุคือ Google Books API (ไม่มี key) โดน rate limit 429 ตั้งแต่รอบแรก แล้วโค้ดเดิม mark ว่า "เช็คแล้ว" ถาวรทั้งที่ยังไม่เคยเช็คสำเร็จจริง เลยไม่มีปกไปตลอดกาล ตรวจสดผ่าน Claude Browser บนเครื่อง ohm เจอ 429 ใน console + `coverChecked:true,coverUrl:null` ค้างใน localStorage จริง → แก้ทั้งชุด: แยกสถานะ done/rate-limited/error ชัดเจน, เปลี่ยนเป็น `coverCheckedAt`(timestamp) auto-retry ทุก 24 ชม., เจอ 429 พักทั้งแอป 10 นาทีไม่ mark เวลา, เว้นจังหวะ 2.5 วิ/เล่มกันยิงรัว, เพิ่มปุ่มกดลองใหม่ด้วยมือที่ไอคอนสำรอง, แก้ stale-closure เดิมด้วย `dataRef`/`persistRef` · smoke test ผ่าน 163/163 · **ยังไม่ยืนยัน 100% เพราะตอนแก้เสร็จ quota ของ ohm ยังไม่คืน (ทดสอบยิงตรงยังเจอ 429)** — ohm push แล้วรอ 10-15 นาทีหรือกดลองใหม่เอง ถ้ายังไม่ขึ้นแจ้งกลับมา · **ohm ยังต้อง push เอง** · ก่อนหน้า 2026-09-14: ข้อ 41 = ปรับปรุง Book Queue รอบ 2: โซน "รายการหนังสือทั้งหมด" เป็น **ชั้นหนังสือ (shelf)** จริง — ปกดึงอัตโนมัติจาก Google Books API (public, ไม่ใช้ Drive/ไม่ใช่ระบบไฟล์แนบตามข้อ 25 เดิม, cache เป็น `coverUrl` ในตัวข้อมูลหนังสือ ซิงก์ผ่าน Drive JSON เดิมได้เลย) + จุดสถานะสี ขาว/เหลือง/เขียว (ต้องอ่าน/กำลังอ่าน/อ่านแล้ว) แทนป้ายคำเดิม · แซนด์บ็อกซ์ทดสอบยิงเน็ตหา googleapis.com ไม่ได้ (บล็อก) — smoke test เช็คได้แค่โครงสร้าง+fallback ไม่ได้ยืนยันปกจริงขึ้น ต้อง ohm เปิดแอปจริงเช็คหลัง push · smoke test ผ่าน 163/163 · ohm ขอเพิ่มอีก 3 เรื่องไว้ทำต่อ (ยังไม่เริ่ม): หน้ารีวิว+ให้คะแนนหนังสือ, รีวิว video/podcast ในหน้าเดียวกัน, ไอเดียเสริม (สถิติการอ่าน/คำคม/แท็ก+ตัวกรอง) · **ohm ยังต้อง push เอง** · ก่อนหน้า 2026-09-14: ข้อ 40 = ข้อ 24/9 **บางส่วน** เสร็จ — เฉพาะ Book Queue (ohm ขอทีละหน้า ไม่รวบทั้ง 4 หน้าตามแผนเดิม) เป็น bento ตามธรรมนูญข้อ 32 + `BookFormModal` ตัวเดียวเพิ่ม/แก้ไข (ปุ่มแก้ไขที่เดิมไม่มี) + เลิก local state มิเรอร์ props (อ่านจาก `data.bookQueue` ตรงๆ เหมือนแท็บ Finance/Tracker) · Health/Documents/Journal **ยังไม่แตะ** · smoke test ผ่าน 162/162 · **ohm ยังต้อง push เอง** · ก่อนหน้า 2026-09-13: ข้อ 39 = ข้อ 24/8 เสร็จแล้ว Income+Expenses **reskin เท่านั้น** (สี/ฟอนต์/ความโค้ง/ระยะ) — คงตาราง/รายการความหนาแน่นสูงไว้ ไม่รื้อเป็น bento ตามที่ ohm ยืนยันสโคปผ่าน AskUserQuestion + ลบ `CategoryPieCard`/`renderPieLabel`/CSS `.fin-pie-*`/`.pie-center-*` ทิ้ง แทนที่ด้วย `FinDonutCard` (การ์ดเดี่ยว ไม่ใช้ `.db-bento`) · smoke test ผ่าน 147/147 · **follow-up วันเดียวกัน:** ohm ขอขยายวงโดนัทหน้านี้ให้ใหญ่ขึ้น → สโคปเฉพาะ `.fin-list-page .db-donut` (ไม่กระทบ `.db-donut` หน้าอื่น) ปรับ 2 รอบ 104px→220px→**340px** (ตามที่ ohm ขอเพิ่ม) · smoke test ผ่าน 149/149 · **ohm ยังต้อง push เอง** · ก่อนหน้า 2026-09-13: ข้อ 38 = ข้อ 24/7 เสร็จแล้ว หน้า Finance Review เป็น bento ตามธรรมนูญข้อ 32 + BudgetPanel ยกไปใช้ DashCollapse · ตรรกะ buildMonthReview/detectAnomalies/budgetRank ไม่ได้แตะ · smoke test ผ่าน 132/132 · ก่อนหน้า 2026-08-27: ข้อ 35 = ปรับสเกลตัวอักษร + ล้างสีธีมม่วงทั้งแอปรอบเดียว + ปุ่มกาง/ย่อรายการยาว + เกราะกันถอยหลังใน smoke test · ผ่าน 84/84 · ก่อนหน้า 2026-08-26: ข้อ 34 = ข้อ 24/4 เสร็จแล้ว Finance Overview เป็น bento + `.inv-page`→`.fin-page` (เปลือกหน้าร่วมของทุกแท็บ Finance) · smoke test ผ่าน 72/72 · **push ขึ้น GitHub Pages แล้ว 2026-08-27 (`069aff6..6e15e58`)** — ohm push เองจากเครื่อง · ข้อ 33 = ข้อ 24/3 เสร็จแล้ว หน้า Investments เป็น bento + ปุ่มเพิ่มข้อมูลปุ่มเดียว + เลิกใช้ Recharts ในหน้านี้ · smoke test ผ่าน 61/61 · **ยังไม่ push** · ข้อ 32 = ธรรมนูญดีไซน์ สเกลตามหน้า Home บังคับตั้งแต่ข้อ 24/3 · ข้อ 27 = ข้อ 24/1 เสร็จแล้ว โครงเมนูทั้งแอป sidebar→แถบแคปซูลบน + สลับสีจริงเป็น Dark Emerald แล้ว (screenshot ยืนยันแล้ว) **push แล้ว** · ข้อ 28 = หลักการเพิ่มเติม การ์ดกระทัดรัด+ปุ่มย่อรายการยาว ใช้ตอนทำแต่ละหน้าจริง · ข้อ 26 = ข้อ 24/0 ปูพื้น CSS variables 94 ตัว · ข้อ 25 = วางสถาปัตยกรรมระบบเก็บไฟล์ Drive เริ่มจากปกหนังสือ รอคิวหลังข้อ 24/9 · ข้อ 24 = ล็อกลำดับรีดีไซน์ 11 ขั้น (0-10) · ข้อ 23 = ล็อกชุดสีธีมใหม่ Dark Emerald + mockup ที่ `mockups/` · ohm ยืนยันว่า**จัดหมวด "อื่นๆ" ครบแล้ว** → ปัญหาข้อ 1 ในข้อ 5 ปิดได้ · ก่อนหน้านี้ 2026-08-25: ข้อ 20 หน้ายืนยัน Import + ข้อ 21 หมุดโปรเจกต์ **push แล้ว** · ข้อ 22 = รันเทสต์จริงย้อนหลัง 4 รอบ ผ่าน 43/43 ไม่มี runtime error)
...
> **เสร็จแล้ว (2026-08-23 → 2026-09-16) — ปรับปรุงหน้า Investments รอบใหญ่ ปิดครบทุกเรื่องที่ค้างไว้:** ประเภทสินทรัพย์ 7 แบบ (เฟส 1-4 เดิม), มูลค่าปัจจุบันกรอกเอง, เป้าหมายลงทุน, **พอร์ตคริปโตรองรับ 2 สกุลเงิน THB/USD** (ข้อ 15), **บันทึกซื้อเพิ่ม/DCA รายงวด + สรุปแยกพอร์ต** (ข้อ 16), **XIRR/ผลตอบแทน %ต่อปี ต่อเหรียญ+รวมพอร์ต** (ข้อ 51), **หน้าประวัติ DCA ต่อเหรียญ + ลบทีละงวด** (ข้อ 52), **เป้าสัดส่วนพอร์ตต่อประเภท + กราฟมูลค่าพอร์ตตามเวลา** (ข้อ 53) — ดูรายละเอียดที่ข้อ 4, 15, 16, 4.4
> **ยังไม่ได้ทำ (ไม่เกี่ยวกับฟีเจอร์ — เป็นเรื่องกรอกข้อมูลจริง):** ohm ยังไม่ได้กรอกข้อมูลพอร์ตคริปโตจริงลงระบบ — 2 พอร์ตจริง `Binance Global` (ถือยาว ไม่ซื้อเพิ่ม) กับ `Binance TH` (DCA ซื้อทุกวัน อัปเดตเดือนละครั้งผ่านปุ่ม "ซื้อเพิ่ม") UI พร้อมกรอกได้เลยตอนนี้ทั้งหมด (รวมฟีเจอร์ข้อ 51-53 ด้วย)
...
32. **ธรรมนูญดีไซน์: ทุกหน้าต้องสเกลตามหน้า Home (ohm สั่ง 2026-08-26 หลังจบข้อ 24/2)** — ohm บอกว่า "หน้า home ลงตัวมาก อยากให้ทุกหน้าสเกลสัมพันธ์กับหน้าโฮม" · **ข้อนี้เป็นข้อบังคับของงานข้อ 24/3 เป็นต้นไปทุกข้อ ไม่ใช่งานแยก** อ่านข้อนี้ก่อนเริ่มรีดีไซน์ทุกหน้า
   ...
   **คิว (ข้อ 24): ~~3 Investments~~ ✅ → ~~4 Finance Overview~~ ✅ → ~~5 Debts~~ ✅ → ~~6 Tracker~~ ✅ → ~~7 Finance Review~~ ✅ → ~~8 Income+Expenses (reskin เท่านั้น)~~ ✅ → **9 หน้าเล็กรวบทีเดียว — กำลังทำ: ~~Book Queue~~ ✅ → Health (ต่อไป) → Documents → Journal** → 10 มือถือ**

33. **ข้อ 24/3 เสร็จแล้ว: หน้า Investments เป็น bento ตามธรรมนูญข้อ 32 (2026-08-26)** — งานแรกที่อยู่ใต้บังคับธรรมนูญข้อ 32 เต็มรูปแบบ

    **โครงหน้าใหม่ (`FinanceInvestmentsTab`, `preview-dashboard.html`):**
    - ห่อทั้งหน้าด้วย `.fin-section.inv-page` → `.db-hero` (หัวข้อ 38px + ตัวเลขใหญ่ 3 ตัว 40px) → `.db-bento` การ์ด 8 ใบ
    - ลำดับการ์ด: Allocation | กำไร/ขาดทุนรายประเภท · เงินที่ใส่รายเดือน | สรุปพอร์ตคริปโต · มูลค่าปัจจุบันรายประเภท | เป้าหมายการลงทุน · เหรียญที่ถืออยู่ (span4) · Savings & Investments (span4)
    - **ลำดับนี้จงใจ** — จับคู่การ์ดสูงใกล้กันไว้ข้างกัน ไม่งั้นจะเหลือช่องว่างใหญ่กลางกริด (เคยเป็นตอนร่างแรก)
    - 4 การ์ดสถิติ `.fin-stat-card` ธีมเก่าถูกยุบเป็นแถบฮีโร่ (ohm เลือกเอง) — "เฉลี่ยต่อเดือน" ย้ายไปอยู่บรรทัด `.db-hero-line`

    **ข้อ 32/2 ปุ่มเดียวต่อหน้า:** `+ เพิ่มเหรียญ` กับ `+ เพิ่มรายการ` ยุบเป็นปุ่ม **"เพิ่มข้อมูล"** ปุ่มเดียว (`.inv-add-btn` ในฮีโร่) กดแล้วเปิดโมดัลเลือก 2 ทาง (รายการสะสม / เหรียญคริปโต) แบบ Quick Capture · **ปุ่ม + DCA บนการ์ดเหรียญยังอยู่ที่เดิม** ohm ยืนยันว่าไม่นับเป็นปุ่มเพิ่มระดับหน้า เพราะเป็นการกระทำเฉพาะเหรียญนั้น

    **ข้อ 32/3 พับได้:** `มูลค่าปัจจุบันรายประเภท` · `เป้าหมายการลงทุน` · `เหรียญที่ถืออยู่` · `Savings & Investments` ใช้ `DashCollapse` หมด — รายการทั้งหมด `defaultOpen={false}` (ยาวสุดในหน้า) ที่เหลือเปิดตามเงื่อนไข (มีค่าค้างอัปเดต / มีข้อมูล)

    **แก้คอมโพเนนต์กลางที่หน้าอื่นใช้ร่วม (backward compatible ทั้งคู่ ค่าปริยายเท่าเดิม):**
    - `DashCollapse` เพิ่ม prop `span` (ปริยาย `db-span2`) และ `actions` (ปุ่มบนหัวการ์ด ห่อ `stopPropagation` ให้แล้ว ไม่งั้นกดแล้วการ์ดพับ)
    - `INVEST_TYPE_COLORS` เปลี่ยนจากม่วง/ชมพูเป็น Dark Emerald และ **`DB_ALLOC_COLORS` เปลี่ยนเป็น alias ของตัวนี้** — ตอนนี้สีประเภทสินทรัพย์มีแหล่งเดียวทั้งแอป อย่านิยามซ้ำอีก
    - เพิ่ม `CRYPTO_COIN_COLORS` (8 สีเขียว-ทองสลับเข้ม/อ่อน) สำหรับโดนัทรายเหรียญ
    - เพิ่มไอคอน `SortDesc` (`fa-arrow-down-wide-short`) ใช้กับปุ่มเรียงตามมูลค่า

    **เลิกใช้ Recharts ในหน้านี้ทั้งหมด** — การ์ด Allocation และมินิพายรายพอร์ตเปลี่ยนไปใช้ `DashDonut` (SVG ล้วน ชุดเดียวกับการ์ด Portfolio หน้า Home) · `Contributions by Month` เปลี่ยนจาก `BarChart` เป็นแท่ง CSS `.db-bars` แบบการ์ด Movement หน้า Home (`.inv-bars` สูง 132px + `margin-top:auto` ให้แท่งเกาะก้นการ์ด ไม่ลอยกลาง) · **Recharts ยังถูกใช้ในหน้าอื่นอยู่ อย่าถอด import**

    **ข้อ 32/4 ฟอนต์:** ไล่ `fontSize` inline ในหน้านี้ออก**หมดแล้ว** ย้ายไปเป็นคลาสใน `styles.css` (`.inv-coin-name` 13 · `.inv-list-name`/`.inv-list-amt` 12.5 · `.pnl-bar-pct`/`.inv-goal-health` 11) · smoke test มีเช็คข้อนี้แล้ว (ยกเว้นไอคอน `makeIcon()` ที่ตั้ง fontSize inline เป็นดีไซน์ของมันเอง ทั้งแอปเป็นแบบนี้)

    **แตะหน้าอื่นด้วย 1 จุด (ตั้งใจ):** `.fin-tab-nav`/`.fin-tab-btn` ย่อจาก **20px → 13px** เป็นแคปซูล `border-radius:999px` + ไอคอนใน `FinancePage` จาก `size={24}` → `size={13}` — กระทบทุกแท็บ Finance (Overview/Review/Income/Expenses/Debts) แต่เป็นเปลือกนอกของหน้าที่กำลังทำ และเดิมใหญ่กว่าสเกลธรรมนูญ 7px **เนื้อในของแท็บอื่นยังไม่แตะ รอคิวข้อ 24/4-24/8 ตามเดิม**

    **CSS ที่เพิ่ม** (ท้าย `styles.css` บล็อก "ข้อ 24/3"): `.inv-page` `.inv-add-btn` `.inv-bars` `.inv-warn(.danger)` `.inv-val-row` `.inv-val-input` `.inv-pnl-item` `.inv-goal` `.inv-goal-health` `.pnl-bar-pct` `.inv-coin-name` `.inv-list-name` `.inv-list-amt` `.inv-list-meta` `.inv-inline-note` `.inv-tag` `.quick-type-row.inv-pick` + override `.crypto-platform-grid`(auto-fit 300px) `.crypto-platform-col` `.inv-item-card` `.inv-item-actions` และ media 1200/760px

    **smoke-test.js อัปเดตแล้ว** — บล็อก `[ข้อ 18]` เดิมเขียนทับเป็น `[ข้อ 24/3]` เพราะเช็คมาร์กอัปเก่า (`.inv-alloc-grid`, `.crypto-mini-pie`, recharts sector) ที่ไม่มีแล้ว · เช็คใหม่ 14 ข้อ รวม: ฮีโร่ 38px + `margin:0` (กันบั๊กข้อ 31 ซ้ำ), ปุ่มเพิ่มข้อมูลต้องเหลือ **ปุ่มเดียว**, ห้ามเหลือคลาสธีมเก่า (`.card`/`.empty`/`.fin-stat-card`) ในหน้านี้, ห้ามเหลือ `fontSize` inline, กดหัวการ์ดพับแล้วต้องกางจริง
    - **ผลรัน: ผ่าน 61/61 · NO RUNTIME ERRORS** (ภายหลังเพิ่มเช็คหน้า Overview เป็น 72/72 ดูข้อ 34) · เปิดจริงด้วย Playwright/Chromium ดู screenshot ทั้งเดสก์ท็อป (1440) และมือถือ (390) แล้ว ไม่ล้นแนวนอน
    - รันในคอนเทนเนอร์ต้องใส่ `CHROME_PATH=/opt/pw-browsers/chromium-1194/chrome-linux/chrome` (Chromium ที่ playwright npm รุ่นใหม่มองหาคนละเวอร์ชันกับที่ติดตั้งไว้)

    **ยังไม่ได้ทำ (ตั้งใจ ไม่ใช่ลืม):** งานข้อ 24/2 ที่ยังค้าง — XIRR, ประวัติ DCA ต่อเหรียญ, ตั้งเป้าสัดส่วนพอร์ต, กราฟมูลค่าพอร์ตตามเวลา — ยังเป็นงานฟีเจอร์แยก ไม่ใช่ส่วนของรีดีไซน์

    **ขั้นต่อไปตามคิวข้อ 24 คือลำดับที่ 4: Finance Overview**

34. **ข้อ 24/4 เสร็จแล้ว: Finance Overview เป็น bento (2026-08-26)** — ต่อจากข้อ 33 ในเซสชันเดียวกัน (ohm สั่ง "ต่อไปเลย")

    **โครงหน้าใหม่ (`FinanceOverviewTab`):** `.fin-section.fin-page` → `.db-hero` (Overview · ภาพรวมการเงิน + ตัวเลขใหญ่ 3 ตัว: รายรับ/รายจ่าย/คงเหลือ) → `.db-bento` การ์ด 4 ใบ = Net Worth | บัญชีเงินสด · รายรับตามแหล่งที่มา | รายจ่ายตามหมวด
    - **ตัวกรอง Month/Year ย้ายเข้าฮีโร่** เป็นแคปซูล `.fin-range-btn` + `.fin-range-select` (เดิมเป็น `.fin-filter-group` ป้าย 17px + select 20px) · เปลี่ยนป้ายเป็นไทย + วันที่เป็น `th-TH` (พ.ศ.)
    - บรรทัดฮีโร่โชว์ **อัตราการเก็บเงิน** (`netIncome/totalIncome`) ที่เดิมไม่มี

    **`.inv-page` เปลี่ยนชื่อเป็น `.fin-page`** — ตอนนี้เป็นเปลือกหน้าร่วมของทุกแท็บ Finance ที่รีดีไซน์แล้ว ไม่ใช่ของหน้า Investments อย่างเดียว **หน้าที่ทำต่อจากนี้ (24/5 เป็นต้นไป) ให้ใส่คลาสนี้ด้วย** · `CRYPTO_COIN_COLORS` เปลี่ยนชื่อเป็น **`DB_CAT_COLORS`** (ชุดสีหมวดหมู่ทั่วไปของธีม ใช้ทั้งโดนัทรายเหรียญและโดนัทรายรับ/รายจ่าย)

    **คอมโพเนนต์ที่เพิ่ม/รื้อ:**
    - **`FinDonutCard`** (ใหม่) — การ์ดโดนัทหมวดหมู่แบบ bento (ปกติใช้พร้อม `span="db-span2"` ในกริด) · ✅ **อัปเดต (ข้อ 39/24-8):** ตอนนี้ใช้แทน `CategoryPieCard` แล้วทั้งในแท็บ Income และ Expenses ด้วย — เรียกโดย**ไม่ส่ง `span`** จึงได้ default `db-span2` แต่เพราะหน้า Income/Expenses ไม่ใช่ `.db-bento` (ไม่ใช่ grid) กฎ `grid-column:span 2` เลยไม่มีผล การ์ดจึงเรนเดอร์เป็นการ์ดเดี่ยวปกติ (ตามที่ ohm อนุมัติผ่าน AskUserQuestion) · `CategoryPieCard`/`renderPieLabel` + CSS `.fin-pie-*`/`.pie-center-*` **ถูกลบทิ้งแล้ว**
    - เลือกหมวดด้วยการ **กดที่ legend** ไม่ใช่คลิกชิ้นพาย (โดนัท SVG ของ `DashDonut` ไม่มี hit area รายชิ้น)
    - **`NetWorthCard` รื้อใหม่** — เหลือแค่การ์ดสรุป (แถบสัดส่วน 2 แถบสเกลเดียวกัน + 3 แถว) ตารางบัญชีเงินสดแยกออกไปเป็น **`CashAccountsCard`** (DashCollapse) · แถว "ลงทุน"/"หนี้" กดแล้วสลับไปแท็บนั้นได้ (รับ `setTab` ที่ส่งลงมาจาก `FinancePage`)
    - **`AddCashAccountModal`** (ใหม่) — ปุ่มเดียวของหน้า "เพิ่มข้อมูล" เรียกตัวนี้ (ข้อ 32/2) ปุ่ม + ในตารางบัญชีเดิมถูกลบ

    **กับดักข้อมูลที่เจอตอนทำ (สำคัญ):** หมวดรายจ่ายที่มีรายการ **คืนเงิน** (`source:"csv-refund"`, amount บวก) มากกว่าที่จ่าย จะได้ยอดสุทธิ **ติดลบ** — โดนัทวาดชิ้นติดลบไม่ได้ (รอบแรกโชว์ "อาหาร -8%") · แก้เป็น: วงโดนัทคิดจากหมวดที่ยอด > 0 เท่านั้น, หมวดติดลบยังอยู่ใน legend แต่โชว์เป็น **จำนวนเงิน** ไม่ใช่ % , แถบ % ใน detail clamp 0-100 และถ้ายอดวง ≠ ยอดสุทธิ จะมีบรรทัดอธิบายใต้โดนัท **ที่อื่นที่วาดพาย/โดนัทจากยอดรายจ่ายต้องระวังเรื่องนี้เหมือนกัน**

    **CSS เพิ่ม** (ต่อท้ายบล็อกเดียวกับข้อ 24/3): `.fin-range` `.fin-range-btn` `.fin-range-select` `.nw-bar` `.nw-row-link` `.fin-legend-click`/`.fin-lg-btn` `.fin-detail`/`.fin-detail-list`/`.fin-detail-item`/`.fin-detail-amt`/`.fin-detail-memo` `.fin-inline-note` `.fin-acc-row`/`.fin-acc-name`/`.fin-acc-when` + `.fin-page .db-bignum-inline{white-space:nowrap}` (จอแคบเคยตัด "-" ออกจาก "฿6,600" คนละบรรทัด) + media 760px ซ่อนวันที่อัปเดตบัญชี

    **smoke-test.js:** เพิ่มบล็อก `[ข้อ 24/4]` 11 เช็ค (โครง bento · ฮีโร่ 38px + margin:0 · ตัวเลขใหญ่ 3 ตัว · แถบ Net Worth + แถวลิงก์ · โดนัทไม่ใช่ Recharts · ตัวกรองแคปซูล · ปุ่มเพิ่มปุ่มเดียว · ไม่เหลือคลาสเก่า/ตาราง · ไม่เหลือ fontSize inline · กด legend แล้วกางรายการ · โมดัลเพิ่มบัญชี) — **ผลรัน: ผ่าน 72/72 · NO RUNTIME ERRORS** · ดู screenshot 1440 + 390 แล้ว
    - ระวัง: ตัวแปร `ov` ถูกใช้ไปแล้วในบล็อกข้อ 20 (import overlay) — บล็อกใหม่ใช้ชื่อ `ovw`

    **ยังไม่แตะ:** เนื้อในแท็บ Income/Expenses/Review/Debts (รอคิว) · `COLORS` ชุดม่วงเดิมยังใช้ในหน้าที่ยังไม่ถึงคิว อย่าเพิ่งลบ

    **ขั้นต่อไปตามคิวข้อ 24 คือลำดับที่ 5: Debts**

    **push แล้ว 2026-08-27** — `45d552a` (ข้อ 33+34) · `2844582` (docs) · `6e15e58` (ข้อ 35) ขึ้น GitHub Pages ครบ working tree สะอาด · สิ่งที่ ohm ต้องเปิดเช็คของจริงที่ https://supakit-ohm.github.io/secretary-ohm/preview-dashboard.html เช็คของจริง โดยเฉพาะ: ไอคอน FontAwesome ขึ้นครบไหม (เครื่องเทสต์บล็อก CDN เลยไม่เห็น), กดปุ่ม "เพิ่มข้อมูล" ทั้ง 2 หน้าแล้วบันทึกเข้าจริงไหม, กดแถว "ลงทุน"/"หนี้" ในการ์ด Net Worth แล้วสลับแท็บไหม, ข้อมูลจริงของ ohm (ไม่ใช่ seed) แสดงถูกไหม

35. **ข้อ 24/6 (แทรกคิว): ปรับสเกลตัวอักษร + ล้างสีธีมม่วง "ทั้งแอป" รอบเดียว (ohm สั่ง 2026-08-27)** — ohm บอกว่า "ขนาดตัวอักษรยังไม่สัมพันธ์กับหน้าโฮม แถบ finance หน้าย่อยๆ ตัวอักษรยังใหญ่ ช่องเลือกเดือนบางอันยังเป็นสีม่วง รายการยาวๆ ไม่ได้ซ่อนให้" · **ohm เลือกเอง: แก้ทั้งแอปรอบเดียว ไม่สโคปเฉพาะ Finance** (คลาสอย่าง `.exp-table`/`.modal-input`/`.card-head`/`.field-label`/`.empty` ใช้ร่วมกับ Tracker/Health/Books/Journal)

    **วิธีหาปัญหา:** เขียน `audit.js` (สคริปต์ชั่วคราวในคอนเทนเนอร์ ไม่ได้เก็บไว้) เปิดแต่ละแท็บด้วย Chromium แล้วไล่ `getComputedStyle` ทุก element รายงาน (1) font-size > 14px (2) สีที่ RGB เข้าข่ายม่วง (b>g+18 && r>g+4) (3) `style.fontSize` inline — **ได้ผลชัดกว่าเดาจากซอร์สมาก** ถ้าต้องทำอีกให้ทำแบบนี้

    **สิ่งที่แก้ (3 ชั้น):**
    1. **`styles.css` — ย่อ font-size 191 จุด** ด้วยตารางแปลง (48→34, 38→30, 27→20, 24→16, 21→13.5, 20→13, 19→13, 18→12.5, 17→12.5, 16→11.5, 15→11) **เฉพาะส่วนก่อนบล็อก `db-`** (บล็อก db- สเกลถูกอยู่แล้ว ห้ามแตะ) แล้วตามด้วย override รายคลาสอีก ~40 คลาสให้ตรงตารางในข้อ 32 เป๊ะๆ (`.card-head` 14 · `.modal-input` 12.5 · `.field-label` 11 · `.exp-table` 12.5 / th 11 · `.empty` 12 · `.fin-stat-value` 26 …)
    2. **`preview-dashboard.html` — hex ธีมม่วง inline 118 จุด → ตัวแปรธีม** (`#a29bc4`→`var(--ink-2)`, `#8b5cf6`→`var(--accent)`, `#191330`→`var(--surface-sunken)`, `#3a3155`→`var(--line)` ฯลฯ) · **ข้อควรระวังที่เจอ:** สีที่ถูกยัดเป็น SVG presentation attribute ของ Recharts (`fill:`/`stroke:`/`stopColor`) **ใช้ `var()` ไม่ได้** ต้องเป็น hex — สคริปต์เลยแทนที่ 2 รอบ: รอบแรก hex→hex สำหรับ context พวกนั้น รอบสองที่เหลือ→`var()` · ย่อ `fontSize` inline 85 จุด และ `size={}` ของไอคอน 170 จุดด้วยตารางเดียวกัน
    3. **สีที่อยู่ใน "ข้อมูล" ไม่ใช่ CSS** — `COLORS` (ชุดสีกราฟ) และ `GOAL_COLORS` เปลี่ยนเป็นชุด Dark Emerald, `DB_CAT_COLORS` กลายเป็น alias ของ `COLORS` · **`projects[].color`/`goals[].color` ที่ ohm บันทึกไว้สมัยธีมม่วงอยู่ใน localStorage** ทำให้การ์ด Tracker ยังม่วงคาอยู่ → เพิ่ม **migration ตอนโหลด** map `#8b5cf6→#10b981`, `#a78bfa→#34d399`, `#c4b5fd→#6ee7b7`, `#f472b6→#d9b26a`, `#5a7a9e→#6f9e8d` (อยู่ในบล็อก migration เดียวกับ dateFixed/invTyped)

    **รายการยาวที่ซ่อนได้แล้ว (ข้อ 32/3):** เพิ่ม `useRowLimit()` + `<ShowMoreRow>` (ปุ่ม "อีก X รายการ" / "แสดงทั้งหมด" / "ย่อกลับ" + ตัวนับ `25/65`) — ใช้กับหน้าที่**ต้องคงตารางไว้** ต่างจาก `DashCollapse` ที่ใช้กับหน้า bento · ใส่แล้วที่: **All Income · All Expenses** (ทีละ 25 แถว) · หน้า **Review** 3 ลิสต์ (หมวดรายจ่าย/รายจ่ายคงที่/รายการผิดปกติ ทีละ 6) · **ไม่ใส่หน้า Debts** เพราะ 1 แถว = 1 หนี้ ยังไงก็ไม่ยาว
    - `ShowMoreRow` ตัดแค่ **แถวที่แสดง** — "เลือกทั้งหมด" และยอดรวมยังคิดจากรายการเต็มตามตัวกรองเหมือนเดิม (อย่าเผลอเปลี่ยนเป็น `visible*`)

    **บั๊กที่เจอระหว่างทางและแก้แล้ว:** `.icon-btn` เป็นปุ่มไอคอนขนาดคงที่ (32×32) แต่ถูกใช้กับปุ่มที่**มีข้อความ** "ยืนยันหักยอดคงเหลือ" ในหน้า Debts → ข้อความล้นทับแถวบน · เพิ่มคลาส **`.pill-btn` / `.pill-btn.primary`** ไว้ใช้กับปุ่มมีข้อความโดยเฉพาะ **ปุ่มมีข้อความห้ามใช้ `.icon-btn` อีก**
    - ย่อกล่องไอคอนตามตัวอักษรด้วย: `.fin-stat-icon` 52→34, `.icon-btn` 44→32, `.nav-icon-btn` 38→32, `.exp-checkbox` 20→14
    - `barColor` ในหน้า Review: หมวดที่ยังไม่ตั้งงบเคยเป็นแถบ**สีฟ้า** `#5a7a9e` → `var(--ink-6)`

    **เพิ่มเกราะกันถอยหลังใน `smoke-test.js` (บล็อก `[ข้อ 24/6]` 12 เช็ค):** ไล่ทุกหน้า (Home/Tracker/Books + Finance ทั้ง 6 แท็บ) แล้ว fail ถ้าเจอ **สีม่วง** หรือ **font-size > 14px ที่ไม่อยู่ใน whitelist** `[15,18,20,22,24,26,30,32,38,40]` (ตัวเลข/หัวข้อที่ตั้งใจให้ใหญ่ตามตารางข้อ 32) — **ถ้าเพิ่มขนาดใหม่ที่ตั้งใจ ให้เติมใน whitelist อย่าลบเช็ค**
    - **ผลรัน: ผ่าน 84/84 · NO RUNTIME ERRORS** · ดู screenshot ครบทุกหน้า (Home/Tracker/Health/Documents/Books/Journal + Finance 6 แท็บ) ยืนยันหน้า Home ไม่เปลี่ยน

    **ohm เปิดของจริงเช็คแล้ว 2026-08-27 — ผ่านหมด** (ไอคอนขึ้นครบ · การ์ด Tracker เปลี่ยนจากม่วงเป็นเขียวเองจริงตาม migration · ปุ่มกาง/ย่อใน All Expenses ทำงาน · ปุ่มในหน้า Debts ไม่ทับกันแล้ว · สเกลตัวอักษรโอเค) — **ฐานสเกล/สีถือว่านิ่งแล้ว งานข้อ 24 ที่เหลือต่อยอดจากตรงนี้ได้เลย**

    **ยังไม่ได้ทำ:** โครงหน้า Review/Income/Expenses/Debts **ยังเป็นของเดิม** — รอบนี้คือ reskin สเกล/สีล้วน ไม่ได้รื้อเป็น bento · คิวข้อ 24 ที่เหลือ (5 Debts → 6 Tracker → 7 Review → 8 Income+Expenses → 9 หน้าเล็ก → 10 มือถือ) **ยังต้องทำตามเดิม** แค่เริ่มจากฐานที่สเกล/สีตรงแล้ว


36. **ข้อ 24/5 เสร็จแล้ว: หน้า Debts เป็น bento ตามธรรมนูญข้อ 32 (2026-08-27)** — ต่อจากข้อ 35 (ฐานสเกล/สีนิ่งแล้ว) ตามคิวข้อ 24 แถวที่ 5

    **โครงหน้าใหม่ (`FinanceDebtsTab`):** `.fin-section.fin-page` → `.db-hero` (Debts · หนี้สิน + ตัวเลขใหญ่ 3 ตัว: หนี้คงเหลือ / ผ่อนขั้นต่ำต่อเดือน / ชำระไปแล้ว) → `.db-bento` การ์ด 5 ใบ
    - ลำดับการ์ด: **สัดส่วนหนี้** (โดนัท `DashDonut` + legend %) | **ภาระต่อเดือน** · **ประมาณการปลดหนี้** (DashCollapse) | **หักยอดจากรายจ่ายที่ผูกไว้** (DashCollapse) · **รายการหนี้** (DashCollapse `span="db-span4"`)
    - **แถวการ์ดสถิติ 4 ใบ (`.fin-stat-card`) ถูกยุบเป็นแถบฮีโร่** เหมือนที่ทำในข้อ 33/34
    - **ตาราง `.exp-table` ถูกลบทิ้งทั้งก้อน** → `.debt-grid` การ์ดต่อหนี้ 1 ก้อน (ชื่อ + ยอดคงเหลือ 18px + แถบ % ชำระแล้วถ้ามี `principal` + ป้าย `.inv-tag` ชนิด/ประเภท/ผ่อน/ดอกเบี้ย/ครบกำหนด/หมวดที่ผูก/วันอัปเดต + ปุ่มแก้ไข-ลบเป็น `.db-chip`)

    **ข้อ 32/2 ปุ่มเดียวต่อหน้า:** การ์ดฟอร์ม "เพิ่มหนี้ใหม่" ที่ค้างกลางหน้าจอ **และ** การแก้ไข inline ในตาราง ถูกยุบเป็น **`DebtFormModal` ตัวเดียวใช้ทั้งเพิ่มและแก้ไข** (`initial` ว่าง = เพิ่ม, มีค่า = แก้ไข) เรียกจากปุ่ม **"เพิ่มข้อมูล"** ในฮีโร่ปุ่มเดียว · ปุ่มแก้ไขบนการ์ดหนี้ไม่นับเป็นปุ่มเพิ่มระดับหน้า (เหมือนปุ่ม + DCA ในข้อ 33)
    - `normalizeDebt()` เป็นที่เดียวที่แปลงค่าฟอร์ม→ค่าที่เก็บจริง (กฎ null/0, `kind==="self"` บังคับ `type:"other"` + ตัด `interestRate`/`dueDay` ทิ้ง) **ทั้งเพิ่มและแก้ไขเรียกตัวนี้** อย่าเขียนกฎซ้ำสองที่เหมือนเดิม
    - `updatedAt` ขยับเฉพาะตอน `currentBalance` เปลี่ยน (พฤติกรรมเดิม ห้ามเปลี่ยนเป็นขยับทุกครั้งที่กดบันทึก)

    **การ์ดใหม่ที่ไม่เคยมี (คำนวณจากข้อมูลเดิมล้วน ไม่ได้เพิ่ม field ใน data.json):**
    - **ภาระต่อเดือน** — `totMin` เทียบ **รายรับเฉลี่ยต่อเดือน (6 เดือนล่าสุดที่มีข้อมูล)** ได้เป็น DSR % + แถบ · เกิน 40% ขึ้น `.inv-warn` เตือนสภาพคล่อง · แถวในการ์ดคือ **หนี้ทุกก้อน** ไม่ใช่เฉพาะก้อนที่ตั้ง `dueDay` (ก้อนที่ยังไม่ตั้งยอดผ่อนต้องเห็นด้วยว่ายังกรอกไม่ครบ) เรียงตามวันครบกำหนดใกล้สุด ก้อนที่ไม่มีวันครบกำหนดไปท้าย
    - `nextDueDate()` — `dueDay` เป็นวันในเดือน ถ้าเลยไปแล้วนับเป็นเดือนหน้า และ **clamp วันสิ้นเดือน** (ตั้ง 31 แล้วเดือน ก.พ. ต้องไม่กลายเป็น 3 มี.ค.)
    - **ประมาณการปลดหนี้** — `Math.ceil(ยอดคงเหลือ ÷ ผ่อนขั้นต่ำ)` **ไม่คิดดอกเบี้ย** เขียนกำกับไว้ในการ์ดแล้ว ใช้ดูลำดับว่าก้อนไหนหมดก่อน ไม่ใช่ตัวเลขจริงจากธนาคาร — ถ้าจะทำให้แม่นต้องคิด amortization แยกเป็นงานใหม่
    - **หักยอดจากรายจ่ายที่ผูกไว้** — ย้าย UI จับคู่ (เฟส 2 ของข้อ 6) ออกจากใต้แถวตารางมาเป็นการ์ดของตัวเอง · **ตรรกะเดิมทั้งหมด: ยังต้องกด "ยืนยันหักยอดคงเหลือ" เองเสมอ ห้ามหักอัตโนมัติ** และ 1 รายจ่าย map ได้แค่ 1 หนี้ (`linkedDebtId`)

    **สีที่ล้างเพิ่ม:** `DEBT_KIND_STYLE.self` เคยเป็นฟ้า `#38bdf8` ของธีมม่วง → ทองตามธีม (`--gold-soft`/`--gold`/`--gold-dim`) · external → ส้มตามธีม (`--orange-soft`/`--orange`) · ยอดคงเหลือในตารางที่เคย hardcode `#fb923c` หายไปพร้อมตาราง

    **CSS เพิ่ม** (ต่อท้ายบล็อกข้อ 24/3-24/4 ใน `styles.css`): `.debt-modal`(620px) `.debt-row-3` `.debt-save-btn` `.debt-lg-name` `.debt-due-when` `.db-row .val.debt-dim` `.debt-note-top` `.debt-match`/`.debt-match-act` `.debt-grid`(auto-fit 280px) `.debt-item`/`.debt-item-head`/`.debt-item-name`/`.debt-name` `.debt-bal`(18px) `.debt-track` `.debt-meta` `.debt-kind` + media 760px (`.debt-grid` 1 คอลัมน์, ฟอร์มในโมดัลเหลือคอลัมน์เดียว)

    **smoke-test.js:** เพิ่มบล็อก `[ข้อ 24/5]` 15 เช็ค — โครง bento 5 ใบ · ฮีโร่ 38px + `margin:0` · ตัวเลขใหญ่ 3 ตัว · โดนัทไม่ใช่ Recharts · หนี้เป็นการ์ดไม่ใช่ตาราง · การ์ดพับได้ ≥3 · ปุ่มเพิ่มปุ่มเดียว · ไม่เหลือคลาสเก่า/ตาราง · **ปุ่มมีข้อความห้ามใช้ `.icon-btn` (กันบั๊กข้อ 35 ซ้ำ)** · ไม่เหลือ `fontSize` inline · กดยืนยันหักยอดแล้ว `currentBalance` ลดจริง (250000→245000) · เปิดโมดัลเพิ่มแล้วบันทึกได้จริง · ปุ่มแก้ไขเปิดโมดัลพร้อมค่าเดิม
    - **ผลรัน: ผ่าน 99/99 · NO RUNTIME ERRORS** · ดู screenshot 1440 + 390 แล้ว ไม่ล้นแนวนอน
    - ระวังชื่อตัวแปรใน smoke test: `ov`(ข้อ 20) / `ovw`(24/4) / `inv`(24/3) ถูกใช้แล้ว — บล็อกนี้ใช้ `dbt`

    **ยังไม่แตะ:** เนื้อในแท็บ Income/Expenses/Review และหน้า Tracker (รอคิว) · `CategoryPieCard` + CSS `.fin-pie-*` ยังต้องอยู่จนกว่าข้อ 24/8 จะเสร็จ (ตามที่ข้อ 34 สั่งไว้)

    **ขั้นต่อไปตามคิวข้อ 24 คือลำดับที่ 6: Tracker** (Projects/Tasks/ProjectDetail/milestones/ProjectCard)


37. **ข้อ 24/6 เสร็จแล้ว: หน้า Tracker เป็น bento ตามธรรมนูญข้อ 32 (2026-08-27)** — ต่อจากข้อ 36 ในเซสชันเดียวกัน (ohm สั่ง "ทำต่อไป") · **หมายเหตุชื่อ: ข้อ 35 ก็เรียกตัวเองว่า "ข้อ 24/6" แต่นั่นคืองานแทรกคิว (สเกล+สีทั้งแอป) คนละงานกับข้อนี้** บล็อกใน smoke test เลยตั้งชื่อ `[ข้อ 24/6 Tracker]` แยกจาก `[ข้อ 24/6]` เดิม

    **หน้าภาพรวม (`TrackerPage`):** `.page-content.trk-page` → `.db-hero` → `.db-bento` การ์ด 3 ใบ
    - ฮีโร่: Tracker · โปรเจกต์และงาน + บรรทัดบอกโปรเจกต์ที่ใกล้ครบกำหนดสุด + **ตัวกรองเป็นแคปซูล `.fin-range-btn`** (ไทย: กำลังทำ/ต้องเร่ง/สำเร็จแล้ว/ทั้งหมด แทน Active/Behind/Done/All) + ตัวเลขใหญ่ 3 ตัว (ความคืบหน้าเฉลี่ย % · โปรเจกต์ที่ทำอยู่ · งานวันนี้ x/y)
    - **แถว `.goal-stat-card` 4 ใบ และ `.tracker-split` (2 คอลัมน์) ถูกลบทิ้ง** → bento: **งานวันนี้** (TodayPanel เป็น `.db-card` แล้ว) | **สถานะโปรเจกต์** (โดนัท) · **โปรเจกต์** (DashCollapse span4)
    - **การ์ดสถานะโปรเจกต์** — ยุบ `projectHealth` 7 แบบเป็น 5 กลุ่ม (`TRK_BUCKETS`: เลยกำหนด/ช้ากว่าแผน/ตามแผน/สำเร็จแล้ว/พักไว้) **กด legend แล้วกรองรายการด้านล่างได้** (ใช้ `.fin-legend-click`/`.fin-lg-btn` ชุดเดียวกับหน้า Overview)
    - **`ShowMoreRow` + `useRowLimit(6)`** ใน DashCollapse โปรเจกต์ (ข้อ 32/3) — **`useRowLimit` ต้องเรียกก่อน `if(openProject) return`** ไม่งั้นผิด rules of hooks (พลาดตอนแรกแล้วแก้)

    **หน้ารายละเอียด (`ProjectDetail`):** ฮีโร่ = ป้ายหมวด + ชื่อโปรเจกต์ 38px + คำอธิบาย + แถวป้ายสถานะ/เหลือกี่วัน/ควรอยู่ที่ % + ตัวเลขใหญ่ 3 ตัว → bento
    - การ์ด: **ความคืบหน้า** (วงแหวนเดิม + วันเริ่ม/กำหนดเสร็จ/สถานะ + สไลเดอร์ manual ถ้าเป็น measureType manual) | **ตัวเลขสำคัญ** (numeric: ค่าปัจจุบัน/เหลืออีก/ความเร็วเฉลี่ย/คาดว่าเสร็จ + `.inv-warn` เตือนถ้าจะช้ากว่ากำหนด) · **กราฟความคืบหน้า** | **หมุดความคืบหน้า** (DashCollapse) · **ประวัติ Check-in** (DashCollapse) | **งานในโปรเจกต์** (DashCollapse)
    - **`.prj-stat-row` 4 ใบถูกลบ** (ยุบเป็นการ์ดตัวเลขสำคัญ + ฮีโร่) · **ตาราง Check-in History ถูกลบ** → `.trk-ci-row` แถวการ์ด (วันที่ไทย + โน้ตเป็น `.inv-tag` + ค่า + ส่วนต่าง + ปุ่มลบ)
    - **ความกว้างการ์ด "งานในโปรเจกต์" คำนวณเอง** (`taskSpan`) = span4 ถ้าการ์ดก่อนหน้าเป็นเลขคู่ ไม่งั้น span2 — เพราะจำนวนการ์ดเปลี่ยนตาม measureType/มีหมุดไหม/มี check-in ไหม ถ้า fix ไว้จะเหลือช่องโหว่ครึ่งแถว
    - **ยังใช้ Recharts `AreaChart` อยู่ในการ์ดกราฟ** (ต่างจากข้อ 33 ที่ถอดออกหมด) — จงใจ เพราะเป็น time series มีแกน ค่าที่ได้จาก `.db-bars` แบบ CSS สู้ไม่ได้ · **อย่าถอด import Recharts**

    **ข้อ 32/2 ปุ่มเดียวต่อหน้า:** ปุ่ม + ในหัวการ์ด Today และหัวการ์ด Projects → ยุบเป็นปุ่ม **"เพิ่มข้อมูล"** ในฮีโร่ เปิดโมดัลเลือก **โปรเจกต์ / งาน** · หน้ารายละเอียดก็เหมือนกัน: ปุ่ม Check-in + ปุ่ม + งานย่อย → ปุ่มเดียว เปิดโมดัลเลือก **บันทึกความคืบหน้า / งานย่อย** (ถ้าไม่ใช่ numeric จะข้ามโมดัลไปเปิดฟอร์มงานเลย เพราะมีทางเดียว) · ปุ่ม "แก้ไขโปรเจกต์" กับ "ลบโปรเจกต์นี้" ไม่นับเป็นปุ่มเพิ่มข้อมูล

    **สีที่ล้างเพิ่ม:** `projectHealth` "กำลังทำ"/"ตามแผน" เคยเป็นฟ้า `#60a5fa` ของธีมม่วง → `#10b981` · "เร็วกว่าแผน" `#34d399` → `#6ee7b7` (ให้ต่างจากตามแผน) · **`GOAL_COLORS` ยังมี `#60a5fa` อยู่ ไม่ได้ลบ** เพราะเป็นสีที่ ohm เลือกเองต่อโปรเจกต์ ไม่ใช่สีของธีม

    **กับดักที่เจอตอนทำ (จำไว้):** `.prj-bar` มี `flex:1` ไว้ใช้ในแถวแนวนอน `.prj-bar-wrap` — พอเอา `.ms-track` ไปวางใน `.db-card` ที่เป็น **flex column** มันยืดสูงเต็มการ์ด (ได้แถบหมุดสูง 78px แทน 10px) → เพิ่ม `.db-card .ms-track{flex:none}` · **คลาสเก่าที่มี `flex:1` ตัวอื่นๆ ก็ต้องระวังแบบเดียวกันตอนย้ายเข้า `.db-card`**

    **CSS เพิ่ม** (ต่อท้ายบล็อกข้อ 24/5): `.trk-page` `.trk-back` `.trk-del-row` `.db-bignum .n .trk-of` `.trk-hero-left` `.trk-cat` `.trk-hero-actions` `.trk-edit-btn` `.trk-pick-note` `.trk-today-track` `.trk-ring-wrap`/`.trk-ring-side`/`.trk-ring-text` `.trk-manual*` `.trk-ci-row`/`.trk-ci-right`/`.trk-ci-diff` `.db-card .prj-grid`/`.db-card .prj-card` `.ms-item-main` `.db-card .ms-track` + media 760px

    **smoke-test.js:** เพิ่มบล็อก `[ข้อ 24/6 Tracker]` 16 เช็ค (ภาพรวม 10 + หน้ารายละเอียด 6) และ **แก้เช็คเดิมในบล็อก `[ข้อ 21]` 2 จุด** ที่อ้างมาร์กอัปเก่า: หัวการ์ดหมุดย้ายจาก `.card-head` → `.db-head` และข้อความนับเปลี่ยนจาก `หมุดความคืบหน้า (1/4)` → `ถึงแล้ว 1 จาก 4 หมุด` · เพิ่มเช็คมือถือ 2 หน้า (Tracker + รายละเอียดโปรเจกต์)
    - **กับดักใน Playwright:** `:has-text("ประวัติ Check-in")` ไป**จับการ์ด "ตัวเลขสำคัญ" ด้วย** เพราะ note ของมันเขียนว่า "คำนวณจากประวัติ check-in 2 ครั้ง" (has-text ไม่สนตัวพิมพ์เล็กใหญ่) → ต้องเจาะ `.db-head:has(.db-title:text-is("..."))`
    - **ผลรัน: ผ่าน 117/117 · NO RUNTIME ERRORS** · ดู screenshot 1440 + 390 ทั้งหน้าภาพรวมและหน้ารายละเอียดแล้ว ไม่ล้นแนวนอน

    **ยังไม่แตะ:** `ProjectModal` / `TaskModal` / `CheckinModal` (โมดัลยังเป็นของเดิม สเกลผ่านเช็คข้อ 35 อยู่แล้ว) · `TaskRow` ยังใช้ `.trk-task-*` เดิม · หน้า Health/Documents/Books/Journal รอคิวข้อ 24/9

    **ขั้นต่อไปตามคิวข้อ 24 คือลำดับที่ 7: Finance Review** (มี BudgetPanel + anomaly detection ต้องเช็ค logic ควบคู่ไปกับ UI)


38. **ข้อ 24/7 เสร็จแล้ว: หน้า Finance Review เป็น bento ตามธรรมนูญข้อ 32 (2026-09-13)** — เซสชัน claude.ai (device_bash บนเครื่อง ohm ใช้ไม่ได้ตอนนั้นเพราะบั๊ก Windows update 8 ก.ย. ที่ Anthropic รู้แล้ว — ใช้วิธี stage ไฟล์เข้าคอนเทนเนอร์ แก้ด้วย Edit แล้ว commit กลับ แทน)

    **`FinanceReviewTab`:** `.card`/`.rv-hero`/`.rv-toolbar` เดิม → `.fin-page` + `.db-hero` (38px + margin:0) + `.db-bignums` 3 ตัว (รับ/จ่าย/คงเหลือ) + `.db-bento` 6 การ์ด: **สรุปเดือนนี้** (ออม/ลงทุน · คุมได้ · คงที่ · งบรวม vs ใช้จริง) · **รายการผิดปกติ** (DashCollapse) · **รายจ่ายที่คุมได้** (DashCollapse span4 เนื้อหาหลัก) · **รายจ่ายคงที่** (DashCollapse ถ้ามี) · **สรุปเดือนนี้ด้วยตัวเอง** (ให้คะแนน+โน้ต ไม่พับ) · **Budget Settings** (DashCollapse)

    **`BudgetPanel`:** เดิมพับเองด้วย `useState` + `.card` → ยกไปใช้ `DashCollapse` ตรงๆ (ปิดเป็นค่าเริ่มต้นเหมือนเดิม)

    **ข้อ 32/2 ปุ่มเดียวต่อหน้า:** หน้านี้ไม่มี "เพิ่มข้อมูล" จริงๆ (เป็นหน้ารีวิว/วิเคราะห์ ไม่ใช่หน้ากรอกข้อมูล) เลยเหลือปุ่ม **"จัดหมวดใหม่"** ปุ่มเดียวในฮีโร่ (เปิด `RecategorizeModal` เดิม) — ไม่ได้เปลี่ยนชื่อเป็น "เพิ่มข้อมูล" เพราะไม่ตรงความหมาย

    **ตรรกะไม่แตะเลยตามที่ข้อ 24 กำชับ:** `buildMonthReview` / `detectAnomalies` / `budgetRank` / `compareByBudgetStatus` / `getBudget` / `saveReview` เหมือนเดิมทุกจุด — แก้แค่ชั้น UI ที่ห่อรอบมัน

    **ไม่ต้องเพิ่ม CSS เลย** ใช้คลาส `db-*`/`rv-*`/`budget-*`/`inv-*` ที่มีอยู่แล้วทั้งหมด — **CSS เก่าที่ตายแล้ว (`.rv-hero`, `.rv-hero-main/-label/-value/-sub/-grid`, `.rv-mini*`, `.rv-toolbar`, `.rv-anom-count`) ยังไม่ได้ลบ** (ไม่มีหน้าไหนใช้ต่อแล้ว แต่ปล่อยไว้เพราะไม่กระทบอะไร ลบทีหลังได้ถ้า ohm อยากเก็บกวาด)

    **smoke-test.js:** เพิ่มบล็อก `[ข้อ 24/7]` 12 เช็ค (โครง bento · ฮีโร่ 38px+margin:0 · ตัวเลขใหญ่ 3 ตัว · ตัวเลือกเดือนยังอยู่ · การ์ดพับได้ ≥3 · ปุ่มเดียว (ไม่มี "เพิ่มข้อมูล" ปลอม) · ไม่เหลือคลาสเก่า/fontSize inline · anomaly detection คำนวณได้ (seed เจอ 2 หมวดใหม่ในเดือน ก.ค.) · รายจ่ายที่คุมได้จัดหมวดถูก · การ์ดพับ/กางสลับได้จริง · บันทึกคะแนน+โน้ตลง localStorage จริง · ปุ่มจัดหมวดใหม่เปิด modal ได้ · Budget Settings กางออกมาแสดงหมวดได้) + เพิ่ม "Review" เข้าไปในลูปเช็คมือถือ 390px (เดิมมีแค่ Overview/Investments/Debts)
    - **ผลรัน: ผ่าน 132/132 · NO RUNTIME ERRORS**

    **ยังไม่ได้ทำ:** ยังไม่ได้เปิดดูจริงบน GitHub Pages (แค่ผ่าน smoke test ในเบราว์เซอร์ headless) — **ohm ยังต้อง `git add` + `git commit` + `git push` เองจากเครื่อง** (AI push เองไม่ได้ตามข้อ 18) แล้วเช็คหน้าตาจริงอีกทีหลัง push

    **ขั้นต่อไปตามคิวข้อ 24 คือลำดับที่ 8: Income + Expenses** (reskin เท่านั้น ห้ามรื้อเป็น bento ตามที่ข้อ 24 ระบุไว้)


39. **ข้อ 24/8 เสร็จแล้ว: Income + Expenses รีสกินตามธรรมนูญข้อ 32 — reskin เท่านั้น ห้ามรื้อเป็น bento (2026-09-13)** — เซสชัน claude.ai เดียวกันต่อจากข้อ 38 (device_bash บนเครื่อง ohm ยังใช้ไม่ได้ — ใช้วิธี stage ไฟล์เข้าคอนเทนเนอร์ แก้ด้วย Edit แล้ว commit กลับเหมือนเดิม)

    **สโคปงานนี้ต่างจากข้อ 34-38:** หน้านี้ถือเป็น**หน้าเสี่ยงพังง่ายสุด** (745+ รายการข้อมูลการเงินจริง) ข้อ 24 เลยล็อกไว้ชัดเจนว่า "reskin เท่านั้น (สี/ฟอนต์/ความโค้ง/ระยะ) ห้ามรื้อเป็น bento — ต้องคงตาราง/รายการความหนาแน่นสูงไว้" — ก่อนลงมือ ใช้ `AskUserQuestion` ถาม ohm ยืนยัน 2 จุดที่ธรรมนูญข้อ 32 เขียนไว้กว้างจนอาจตีความชนกับสโคปนี้ ohm เลือก **(ก) CSS-only reskin (แคบสุด เสี่ยงน้อยสุด)** — ไม่ใส่ `.db-hero` ไม่รวบปุ่ม คง JSX/ตรรกะเดิมทั้งหมด แค่เปลี่ยนคลาส/สี/ฟอนต์/ระยะ และ **(ข) แทน `CategoryPieCard` ด้วย `FinDonutCard`** (โดนัท SVG ชุดเดียวกับหน้าอื่น ไม่ใช่ Recharts)

    **`FinanceIncomeTab` / `FinanceExpensesTab`:** root `<div className="fin-section">` → เพิ่ม `fin-page fin-list-page` (ไม่ใส่ `.db-hero`/`.db-bento` ตามที่ตกลง) · `.card`/`.card-head`/`.empty` (เดิม) → `.db-card`/`.db-head`+`.db-title`/`.db-empty` · ตาราง `.exp-table` + `TxnFilterBar` + `ShowMoreRow` + `TxnEditModal` **ไม่แตะเลย** (คงความหนาแน่นไว้ตามคำสั่ง) — สองตัวนี้ตรวจแล้วว่าใช้คลาส `db-*` ที่ compliant อยู่แล้วตั้งแต่แรก
    - **Month picker** (`<select>` เดือน) เปลี่ยนจาก inline style → `.field-label` + `.fin-range-select` ห่อด้วย `.fin-month-picker` (คลาสใหม่)
    - **"[Income/Expenses] by [Source/Category] (all time)"** แถวสรุปที่เคย inline style ล้วน → คลาสใหม่ `.fin-agg-row`/`.fin-agg-row-top`/`.fin-agg-name`/`.fin-agg-amt`/`.fin-agg-bar`/`.fin-agg-bar-fill`
    - **Expenses เพิ่ม 2 แบนเนอร์เตือน** (หมวด "อื่นๆ" ค้าง / รายการ "ออม ลงทุน" หลุดมาเป็นรายจ่าย) เดิม inline style ล้วน → คลาสใหม่ `.fin-banner.info` / `.fin-banner.warn` (สีอิงจากธีมเดิมของแต่ละแบนเนอร์: info = `--accent`, warn = คัดลอกพาเลตต์จาก `.inv-warn` ที่มีอยู่แล้ว คือ `--gold`/`--gold-dim`/`--gold-soft`)
    - **CSV upload box** (Expenses) เดิม inline style → คลาส `.fin-csv-box`
    - สีฮาร์ดโค้ดที่เจอในสองแท็บนี้ (`#34d399`, `#fb923c`, `#fbbf24`) → เปลี่ยนเป็น `var(--accent-soft)` / `var(--orange-soft)` ทั้งหมด (แต่**ไม่แตะ**สีฮาร์ดโค้ดในโมดัลอื่นที่ไม่ใช่สโคปนี้ เช่น `DepositReviewModal`/`InvestmentReviewModal`/`SplitExpenseModal`)

    **ลบโค้ดตายแล้วทิ้ง:** ฟังก์ชัน `CategoryPieCard` (Recharts pie เดิม) + `renderPieLabel` (ไม่เคยถูกเรียกอยู่แล้ว) ออกจาก `preview-dashboard.html` ทั้งหมด, CSS `.fin-pie-*` (17 คลาส + media query 2 บล็อก) และ `.pie-center-overlay`/`.pie-center-total`/`.pie-center-label` ออกจาก `styles.css` — **`.pie-detail-*` (ไม่มี prefix "fin-") ปล่อยไว้ตามเดิม** เพราะเช็คแล้วเป็นโค้ดตายจากเวอร์ชันเก่ากว่านั้นอีก ไม่เกี่ยวกับงานนี้

    **`FinDonutCard` เรียกแบบไม่ส่ง `span`** จึงได้ default class `db-span2` แต่เพราะหน้านี้ไม่ใช่ `.db-bento` (ไม่ใช่ CSS grid) กฎ `grid-column:span 2` เลยไม่มีผลอะไร — เรนเดอร์เป็นการ์ดเดี่ยวตามที่ ohm อนุมัติ ไม่ต้องแก้ signature ของ component

    **ตรรกะไม่แตะเลยตามที่ข้อ 24 กำชับ:** `addIncome`/`removeIncome`/`removeSelectedIncome`/`toggleSelectId`/`saveEdit` (Income) และ `addExpense`/`removeExpense`/`removeSelectedExpenses`/`handleCSVUpload`/`applyDeposits`/`applyInvestments`/`closeInvestReview`/`fixStrayInvestments`/`applySplit`/`dismissVague` (Expenses) เหมือนเดิมทุกจุด — แก้แค่ชั้น UI ที่ห่อรอบมัน

    **CSS เพิ่ม** (ต่อท้ายบล็อก `.inv-pnl-item`/`.inv-goal` ใน `styles.css`): `.fin-list-page .db-card/.fin-banner/.fin-month-picker`(margin-bottom) `.fin-month-picker`(max-width) `.fin-banner`(.info/.warn) `.fin-agg-row`(-top/-name/-amt/-bar/-bar-fill) `.fin-csv-box`

    **smoke-test.js:** เพิ่มบล็อก `[ข้อ 24/8]` 15 เช็ค ต่อทั้ง Income และ Expenses (ใช้ `.fin-page` แต่ **ไม่ใช่** `.db-bento` · ตาราง `.exp-table` ยังอยู่ (คงความหนาแน่น) · โดนัท `DashDonut` แทน `CategoryPieCard` เดิม (ไม่ใช่ Recharts) · ไม่เหลือคลาสธีมเก่า (`.card`/`.empty`/`.fin-pie-*`) · การ์ด `db-card` ครบ 4 ใบ (โดนัท + ฟอร์มเพิ่ม + สรุป all-time + ตารางรวม) · ไม่เหลือ `fontSize` inline · ปุ่ม Import Payslip/CSV ยังอยู่ครบ (สโคปนี้ไม่รวบปุ่ม) · แบนเนอร์ใช้คลาสใหม่ไม่ใช่ inline style · เพิ่มรายรับ/รายจ่ายจากฟอร์มแล้วเข้า localStorage จริง) — เจอบั๊กระหว่างทำ 1 จุด: แถวสรุป all-time ใช้ `<span className="db-mono">` เฉยๆ ไม่มีคลาส font-size กำหนด ทำให้สืบทอด 16px จาก UA default (นอก whitelist ข้อ 24/6) แก้โดยเพิ่มคลาส `.fin-agg-amt{font-size:12.5px}`
    - **ผลรัน: ผ่าน 147/147 · NO RUNTIME ERRORS**

    **ตามด้วย follow-up เดียวกันวัน (2026-09-13):** ohm ขอให้วงโดนัทหน้า Income/Expenses ใหญ่ขึ้น "ครึ่งนึงของการ์ด" — เพราะการ์ดหน้านี้เต็มความกว้างหน้าจอ (~1186px ไม่ใช่ span2 ~585px แบบ Overview/Debts/Investments) 50% ตรงๆ จะได้วงใหญ่เกิน (~590px) เลยถาม ohm ผ่าน `AskUserQuestion` 2 ข้อ (ขอบเขต: เฉพาะ Income/Expenses ไม่กระทบหน้าอื่น · ขนาดสูงสุด: จำกัดเพดานไว้) ohm เลือก **เฉพาะ Income/Expenses + จำกัดเพดาน** — เพิ่ม CSS สโคป `.fin-list-page .db-donut{width:50%;max-width:...px;min-width:140px;height:auto;aspect-ratio:1/1}` (ไม่แตะ `.db-donut` ฐาน 104px ที่หน้า Overview/Debts/Investments ยังใช้อยู่) + ขยายตัวเลขกลางวงตามไปด้วย
    - รอบแรก: เพดาน **220px** (`.v` 15→20px, `.k` →11px) — smoke test ผ่าน 149/149
    - ohm ขอใหญ่กว่านี้อีก → ขยับเพดานเป็น **340px** (`.v` →26px, `.k` →13px) เท่านั้นที่เปลี่ยน (ไม่มีผลต่อโครงสร้าง/ตรรกะ) เพิ่มเช็คใน smoke-test.js ล็อกไว้ว่าวงต้องกว้าง 140-340px (วัดจริงได้ 340px ทั้งสองแท็บ เพราะการ์ดกว้างเกินจน 50% ชนเพดาน)
    - **ผลรัน (หลังปรับครั้งล่าสุด): ผ่าน 149/149 · NO RUNTIME ERRORS**

    **ยังไม่ได้ทำ:** ยังไม่ได้เปิดดูจริงบน GitHub Pages (แค่ผ่าน smoke test ในเบราว์เซอร์ headless) — **ohm ยังต้อง `git add` + `git commit` + `git push` เองจากเครื่อง** แล้วเช็คหน้าตาจริงอีกทีหลัง push โดยเฉพาะหน้านี้ที่มีข้อมูลจริง 745+ รายการ ควรเปิดดูให้ครบทั้ง 2 แท็บก่อนมั่นใจ (รวมถึงเช็ควงโดนัทที่เพิ่งขยายด้วย)

    **ขั้นต่อไปตามคิวข้อ 24 คือลำดับที่ 9: หน้าเล็กที่ใช้ไม่บ่อย รวบทำทีเดียว** (Health, Documents, Book Queue, Journal)


40. **ข้อ 24/9 (บางส่วน) เสร็จแล้ว: หน้า Book Queue เป็น bento ตามธรรมนูญข้อ 32 (2026-09-14)** — เซสชัน claude.ai ใหม่ (ohm พิมพ์ "ขึ้นแชทใหม่ทำเรื่อง Book Queue") device_bash บนเครื่อง ohm ยังใช้ไม่ได้เหมือนเดิม — ใช้วิธี stage ไฟล์เข้าคอนเทนเนอร์ แก้ด้วย Edit แล้ว commit กลับเหมือนข้อ 38-39 · **ohm ขอเฉพาะ Book Queue ก่อน ไม่ใช่ทั้ง 4 หน้าในข้อ 9 พร้อมกัน** — Health/Documents/Journal ยังเป็นของเดิม ยังไม่ได้แตะ

    **`BookQueuePage`:** `.page-content`>`.card`(เดิม) → `.page-content book-page` (เปลือกหน้าตามแบบ `.trk-page`/`.fin-page`) → `.db-hero` (ชื่อ "Book Queue · คิวหนังสือ" 38px + margin:0 + hero-line สรุปจำนวนต่อสถานะ) + `.db-bignums` 3 ตัว (ต้องอ่าน/กำลังอ่าน/อ่านแล้ว) → `.db-bento` 3 การ์ด: **กำลังอ่าน** (span2, แถวหนังสือ + ปุ่ม "อ่านจบแล้ว") · **ต้องอ่าน** (span2, เรียงตามความสำคัญมากไปน้อย + ปุ่ม "เริ่มอ่าน") · **รายการหนังสือทั้งหมด** (`DashCollapse` span4, การ์ดต่อเล่มพร้อมป้ายสถานะสี + ปุ่มแก้ไข/ลบ, เปิดอัตโนมัติถ้า ≤6 เล่ม)

    **ข้อ 32/2 ปุ่มเดียวต่อหน้า:** ฟอร์มเพิ่มที่เคยค้างกลางหน้าจอ (toggle แสดง/ซ่อน ไม่มีปุ่มแก้ไขเลย) ถูกยุบเป็น **`BookFormModal` ตัวเดียวใช้ทั้งเพิ่มและแก้ไข** (`initial` ว่าง = เพิ่ม, มีค่า = แก้ไข) เรียกจากปุ่ม **"เพิ่มข้อมูล"** ในฮีโร่ปุ่มเดียว เหมือนแบบ `DebtFormModal` ในข้อ 36 · เพิ่มปุ่มแก้ไข (`.db-chip`) ต่อเล่มในการ์ดรายการทั้งหมด ที่เดิมไม่มี (มีแค่ปุ่มเลื่อนสถานะ + ลบ) — เป็นการเพิ่ม UI ที่สมเหตุสมผลตอนรื้อทั้งหน้าอยู่แล้ว ไม่ใช่ฟีเจอร์นอกสโคป

    **เก็บกวาดที่ทำไปด้วยระหว่างรื้อ (ไม่ใช่แค่เปลี่ยนสี):** เดิม `BookQueuePage` ใช้ `useState(data.bookQueue||[])` เป็น local state สำเนาจาก props (บั๊กแฝง: ถ้า `data` เปลี่ยนจากที่อื่น เช่น sync จาก Google Drive ค่า local จะไม่อัปเดตตาม) → เปลี่ยนเป็นอ่านจาก `data.bookQueue||[]` ตรงๆ ทุก render เหมือนแท็บ Finance/Tracker ทั้งหมด (`persist({...data,bookQueue:next})` เป็นจุดเดียวที่เขียน) — แพตเทิร์นเดียวกับ `FinanceDebtsTab`/`FinanceInvestmentsTab` ที่ใช้อยู่แล้วทั่วแอป ไม่ใช่ของใหม่ที่เสี่ยง

    **CSS เพิ่ม** (ต่อท้ายบล็อกข้อ 24/6 Tracker ใน `styles.css`): `.book-page`(padding เหมือน `.trk-page`/`.fin-page`) `.book-grid`(auto-fit 220px) `.book-item`/`.book-item-head`/`.book-item-name` `.book-meta` `.book-status-badge`(.to-read/.reading/.done สีตามธีมเดิม blue/orange/accent) `.book-stars` + media 760px (`.book-page` ลด padding, `.book-grid` 1 คอลัมน์) — ที่เหลือใช้คลาสร่วม `db-*`/`pill-btn`/`inv-add-btn`/`inv-item-actions`/`db-chip`/`db-row`/`db-tmain`/`db-tname`/`db-tsub` ที่มีอยู่แล้วทั้งหมด ไม่ต้องคิดใหม่

    **ตรรกะ CRUD หลักไม่เปลี่ยน:** เพิ่ม/แก้ไข/ลบยังเขียนลง `data.bookQueue` ตรงๆ เหมือนเดิม ปุ่มเลื่อนสถานะ (`to-read→reading→done`) ยังเป็นตรรกะเดิมเป๊ะ แค่ย้ายมาเป็นปุ่ม `.pill-btn` ในแถว `.db-row` แทนปุ่ม inline-style เดิม

    **smoke-test.js:** เพิ่มบล็อก `[ข้อ 24/9 Book Queue]` 13 เช็ค (โครง `.book-page`+bento 3 การ์ด · ฮีโร่ 38px+margin:0+ชื่อถูกต้อง · ตัวเลขใหญ่ 3 ตัว · ปุ่มเดียว (ไม่มี "เพิ่มข้อมูล" ปลอม) · ไม่เหลือคลาสเก่า/fontSize inline · การ์ดกำลังอ่านโชว์ seed จริง · รายการทั้งหมดครบ 2 เล่มพร้อมป้ายสถานะ · กด "อ่านจบแล้ว" แล้ว status เปลี่ยนจริงใน localStorage · เพิ่ม/แก้ไข/ลบหนังสือจากโมดัลทำงานจริงทั้งหมด)
    - **ผลรัน: ผ่าน 162/162 · NO RUNTIME ERRORS**

    **ยังไม่ได้ทำ:** ยังไม่ได้เปิดดูจริงบน GitHub Pages — **ohm ยังต้อง `git add` + `git commit` + `git push` เองจากเครื่อง** แล้วเช็คหน้าตาจริงอีกที · **Health/Documents/Journal ยังเป็นของเดิมทั้งหมด** (ข้อ 9 ยังไม่จบ เหลืออีก 3 หน้า) · item 25 (แนบไฟล์ปกหนังสือผ่าน Google Drive บนหน้านี้) ยังไม่เริ่ม รอ ohm สั่งเพิ่มเติม (ตามที่ข้อ 25 ระบุไว้ว่าให้ถามก่อน)

    **ขั้นต่อไปตามคิวข้อ 24 (ต่อในข้อ 9 เดิม):** Health → Documents → Journal (ทำทีละหน้าได้ ไม่ต้องรวบทีเดียวเหมือนที่วางแผนไว้เดิม ถ้า ohm อยากแยกเซสชัน) แล้วค่อยไปข้อ 10 มือถือ

41. **ปรับปรุงฟีเจอร์ Book Queue รอบ 2: ชั้นหนังสือ (shelf) + ปกอัตโนมัติ + จุดสถานะสี (2026-09-14, ต่อจากข้อ 40 วันเดียวกัน)** — ohm พิมพ์ "push เรียบร้อย // ฉันต้องการปรับปรุงฟีเจอร์ book queue โดยขึ้นแชทใหม่" แล้วขอต่อในเซสชันเดิม (เหมือนแพตเทิร์นข้อ 40) ถามสโคปผ่าน `AskUserQuestion` 4 ข้อก่อนแตะโค้ด (วิธีได้ปก / ตำแหน่ง video-podcast / ลำดับงาน / ไอเดียเสริม) เพราะ ohm ขอมาหลายเรื่องพร้อมกันในข้อความเดียว — ohm เลือกทำ **เฉพาะ shelf+ปก+จุดสถานะก่อน** รอบนี้

    **สิ่งที่ทำจริงรอบนี้ — เฉพาะโซน "รายการหนังสือทั้งหมด":** เดิมเป็น `.book-grid`/`.book-item` (การ์ดข้อความ + `.book-status-badge` เป็นคำ) → เปลี่ยนเป็น **`.book-shelf`/`.book-shelf-item`** ชั้นหนังสือจริง: ปกหนังสือ (`.book-shelf-cover`, aspect-ratio 2:3) + ชื่อ/ผู้แต่ง (`.db-tname`/`.db-tsub` เดิม) + **จุดสถานะสีมุมขวาบนของปก** (`.book-shelf-dot` — ขาว=ต้องอ่าน / เหลือง=กำลังอ่าน / เขียว=อ่านแล้ว ตามที่ ohm ขอเป๊ะ แทนป้ายคำเดิม) ปุ่มแก้ไข/ลบยังอยู่เหมือนเดิมใต้การ์ด · การ์ด "กำลังอ่าน"/"ต้องอ่าน" (แถว `.db-row`) ด้านบนไม่แตะ ยังเหมือนข้อ 40

    **ปกหนังสืออัตโนมัติ (แทนที่แผนเดิมข้อ 25 สำหรับเคสนี้โดยเฉพาะ):** ohm เลือกตัวเลือก "ดึงอัตโนมัติจาก Google Books API" แทนการอัปโหลดเอง — **ไม่ได้สร้างระบบไฟล์แนบ Drive ใหม่ตามที่ข้อ 25 วางแผนไว้เดิม** แต่ใช้ `fetch()` ไป `https://www.googleapis.com/books/v1/volumes?q=intitle:...+inauthor:...` (public API ไม่ต้องมี API key, ไม่ต้องขอ OAuth scope เพิ่ม) ได้ลิงก์ปกมาเก็บเป็น `coverUrl` ในตัว record หนังสือเลย (ฝังใน `data.bookQueue` ปกติ) → **ซิงก์ผ่าน Google Drive JSON เดิมได้ทันทีโดยไม่ต้องแก้ระบบซิงก์เลย** ฟังก์ชัน `fetchBookCover(title,author)` วางไว้เหนือ `BookFormModal` ใน `preview-dashboard.html` มี `coverChecked:true` กันยิงซ้ำ (ดึงครั้งเดียวต่อเล่ม แม้ดึงไม่เจอก็ mark checked ไม่วนซ้ำ) ดึงทีละเล่มผ่าน `useEffect` คิวเรียงตาม `uncoveredCount` ใน `BookQueuePage` — ถ้าดึงไม่เจอ/ออฟไลน์/พังก็ fallback เป็นไอคอนหนังสือ+ชื่อแทนเงียบๆ ไม่มี error โผล่

    **ข้อควรรู้สำหรับ ohm:** แซนด์บ็อกซ์ทดสอบของ Claude (คอนเทนเนอร์คลาวด์) **ไม่มีสิทธิ์ยิงเน็ตออกไปหา `www.googleapis.com`** (เช็คด้วย curl แล้วบล็อกจริง) ดังนั้น smoke test รอบนี้ทดสอบได้แค่ "โครงสร้าง + fallback path" (จุดสถานะ, กริดชั้นหนังสือ, ปุ่มแก้ไข/ลบ) **ไม่ได้ยืนยันว่าดึงปกจริงสำเร็จ** — ต้องรอ ohm เปิดแอปจริงบนเบราว์เซอร์ (มีเน็ตเต็ม) หลัง push แล้วดูว่าปกขึ้นจริงไหม ถ้าไม่ขึ้นเลยสักเล่ม ให้บอกกลับมาเพื่อ debug (เช่น CORS/query ไม่ตรง/ชื่อเรื่องแปลกดึงไม่เจอ)

    **CSS เปลี่ยน:** ลบ `.book-grid`/`.book-item`/`.book-item-head`/`.book-item-name`/`.book-meta`/`.book-status-badge`/`.book-stars` ทิ้งทั้งหมด (ไม่มีที่ใช้อื่นแล้ว เช็คแล้ว) แทนที่ด้วย `.book-shelf`(grid auto-fill 120px) `.book-shelf-item`/`.book-shelf-cover`(ปก+placeholder gradient)/`.book-shelf-fallback`/`.book-shelf-dot`(.to-read/.reading/.done)/`.book-shelf-info`/`.book-shelf-actions` + media 760px ปรับกริดเป็น 100px

    **DashCollapse:** เปลี่ยน `defaultOpen={books.length<=6}` → `defaultOpen={true}` เสมอ (ชั้นหนังสือเป็นภาพ ควรเห็นทันทีไม่ควรพับซ่อน)

    **smoke-test.js:** แก้บล็อกข้อ 24/9 เดิมให้ตรงคลาสใหม่ (`.book-shelf-item`/`.book-shelf-cover`/`.book-shelf-dot` แทน `.book-item`/`.book-status-badge`) เพิ่มเช็คสีจุดสถานะตรงกับ seed (bk1=reading, bk2=done) — **ผลรัน: ผ่าน 163/163 · NO RUNTIME ERRORS**

    **ยังไม่ได้ทำ (ohm ขอไว้ในคำตอบ แต่ยังไม่เริ่ม รอบหน้า):**
    - หน้ารีวิวหนังสือ: กดเข้าไปเขียนรีวิว/บันทึกเนื้อหาสำคัญ + ให้คะแนนหนังสือ (ต้องมี detail modal/page ใหม่)
    - รีวิว/สรุป video-podcast — ohm เลือกให้อยู่ **ในหน้า Book Queue เดิม** (ไม่แยกหน้าใหม่) แต่ยังไม่ได้ออกแบบโครงสร้างข้อมูล/UI
    - ไอเดียเสริมที่ ohm เลือกไว้: สถิติการอ่าน (จำนวนเล่ม/ปี + เป้าหมายประจำปี), เก็บคำคมที่ชอบจากหนังสือ, แท็ก/หมวดหนังสือ + ตัวกรอง
    - ยังไม่ได้เปิดดูจริงบน GitHub Pages — **ohm ยังต้อง `git add`+`commit`+`push` เองจากเครื่อง** แล้วเช็คว่าปกหนังสือขึ้นจริงก่อนไปต่อ
    - Health/Documents/Journal (ข้อ 9 เดิม) ยังไม่แตะเลย

    **ขั้นต่อไป:** รอ ohm push+เปิดดูจริงยืนยันว่าปกขึ้น แล้วค่อยถามลำดับต่อว่าจะทำรีวิว/เรตติ้ง หรือ video/podcast หรือไอเดียเสริมอันไหนก่อน (ยังไม่ได้ตกลงลำดับหลัง shelf)

42. **แก้บั๊ก: ปกหนังสือไม่ขึ้นเลยสักเล่ม (2026-09-14, ต่อจากข้อ 41 วันเดียวกัน)** — ohm push ข้อ 41 แล้วส่งสกรีนช็อตมาว่าปกไม่ขึ้นเลย ("Atomic habits" ขึ้นแต่ไอคอนสำรอง) เข้าไปตรวจสดผ่านเบราว์เซอร์บนเครื่อง ohm จริง (เปิด `https://supakit-ohm.github.io/secretary-ohm/preview-dashboard.html` ด้วย Claude Browser) เจอทันทีว่า console error `429` และ `localStorage` มี `bookQueue` ทั้ง 2 เล่ม (Atomic Habits, The Lean Startup) ค้างเป็น `coverChecked:true, coverUrl:null` ถาวร

    **ต้นเหตุ:** Google Books API แบบไม่ใส่ API key มี quota ต่อ IP ต่ำมาก โดน rate limit (`429`) ตั้งแต่รอบแรกที่มีหลายเล่มให้เช็คพร้อมกัน (ไล่ยิงรัวไม่มีหน่วง) — โค้ดเดิมใน `fetchBookCover`/effect ของข้อ 24/9b **ไม่แยกแยะ** ระหว่าง "เช็คแล้วไม่เจอปกจริงๆ" กับ "เช็คไม่สำเร็จเพราะโดนบล็อกชั่วคราว" เลย mark `coverChecked:true` ทั้งสองเคสเหมือนกัน → เล่มที่โดน 429 ตอนแรกจะไม่มีปกไปตลอดกาล ไม่มีทางแก้เองได้เลยแม้เน็ต/quota จะกลับมาปกติแล้วก็ตาม — ยืนยันด้วยการยิง `fetch` ตรงจากคอนโซลเบราว์เซอร์จริงของ ohm เห็น `429` เหมือนกัน (ช่วงเวลาที่ตรวจ quota ยังไม่คืน)

    **วิธีแก้ (`fetchBookCover` + effect ใน `BookQueuePage`, ยกเครื่องใหม่ทั้งชุด):**
    - `fetchBookCover` คืนค่าเป็น `{status:'done'|'rate-limited'|'error', url?}` แทนการคืนแค่ url/null เฉยๆ — แยก 3 สถานะชัดเจน
    - เปลี่ยนจาก field `coverChecked`(boolean ถาวร) → **`coverCheckedAt`(timestamp)** — ถ้าเช็คแล้วไม่เจอปกจริง (status `done` แต่ `url:null`) จะ **auto-retry ใหม่ทุก 24 ชม.** เผื่อ Google index เพิ่มทีหลังหรือ ohm แก้ชื่อ/ผู้แต่งให้ตรงขึ้น
    - เจอ `429` → ตั้ง `bookCoverBackoffUntil` (module-level, พักทั้งแอป 10 นาที) ไม่ mark เวลาเล่มนั้นเลย และ**หยุดทั้งคิวการเช็ครอบนี้ทันที** (ไม่ทำให้เล่มอื่นพังตามไปด้วย)
    - เจอ error อื่น (เน็ตสะดุดชั่วคราว) → ข้ามเล่มนั้นไปก่อน ไม่บันทึกเวลา จะลองใหม่ตอนเปิดหน้าครั้งหน้า
    - เพิ่ม `sleep(2500)` หน่วง 2.5 วินาทีระหว่างเช็คแต่ละเล่ม (กันยิงรัวจนโดน 429 ตั้งแต่ต้นเวลามีหลายเล่มพร้อมกัน)
    - เปลี่ยน effect จากแบบ dependency-triggered (`[uncoveredCount]`) เป็น **self-scheduling async loop ตัวเดียวตอน mount** (`useEffect(...,[])` + `while` loop ข้างในมี `cancelled` guard ตอน unmount) — เดิมมีบั๊กแฝงอีกจุดคือ effect เก่าปิด closure ทับ `data` ตอนสร้าง effect ซึ่งอาจ stale ถ้ามีการ persist อื่นแทรกระหว่างรอ fetch (เสี่ยงเขียนทับข้อมูลที่เพิ่งเปลี่ยนไปโดยไม่ตั้งใจ) → แก้ด้วย `dataRef`/`persistRef` (useRef sync ทุก render) ให้ loop อ่าน/เขียนจากค่าล่าสุดเสมอ ไม่ใช่ค่า ณ ตอนสร้าง effect
    - เพิ่ม **ปุ่มลองใหม่ด้วยมือ**: กดที่ไอคอนสำรอง (`.book-shelf-fallback`) เรียก `retryCover(b)` ยิงเช็คทันที 1 ครั้ง (เคารพ backoff ถ้ายังอยู่ในช่วงพัก) — ให้ ohm สั่งลองใหม่เองได้ทันทีไม่ต้องรอ 24 ชม. หรือรีเฟรชหน้า

    **CSS:** เพิ่ม `cursor:pointer` ให้ `.book-shelf-fallback` (บอกว่ากดได้)

    **สิ่งที่ตรวจสอบแล้ว ไม่ใช่สาเหตุ:** Content-Security-Policy (ไม่มี meta CSP ในไฟล์), Service Worker (`sw.js` มีเช็ค `if(url.includes("googleapis.com")) return;` ปล่อยผ่านให้เบราว์เซอร์จัดการเองอยู่แล้ว ไม่ใช่ตัวบล็อก), CORS (เบราว์เซอร์เชื่อม Google Books API สำเร็จ ได้ response กลับมาปกติ แค่ status เป็น 429)

    **smoke-test.js:** ไม่ต้องแก้ (บล็อกทดสอบเดิมของข้อ 24/9b ยังผ่านหมด เพราะทดสอบแค่โครงสร้าง+fallback ไม่ได้ผูกกับผลการดึงปกจริง) — รันซ้ำ **ผ่าน 163/163 · NO RUNTIME ERRORS**

    **ยังไม่ยืนยัน 100%:** ตอนแก้เสร็จ quota ของ ohm ยังไม่คืน (ทดสอบยิง fetch ตรงจากคอนโซลเห็น 429 อยู่) เลย**ยังไม่เห็นปกจริงขึ้นสด** แต่ยืนยันแล้วว่าโค้ดจะไม่ค้างถาวรแบบเดิมแน่นอน (auto-retry 24 ชม. + ปุ่มลองใหม่ + เว้นจังหวะกัน 429 ซ้ำ) — **ขั้นต่อไป: ohm push แล้วรอสัก 10-15 นาที (หรือกดไอคอนสำรองลองใหม่เอง) ถ้ายังไม่ขึ้นอีกให้แจ้งกลับมาเช็คต่อ** (อาจต้องพิจารณาใส่ API key ของ Google Books ถ้า quota แบบไม่มี key ตึงเกินไปสำหรับขนาดคลังหนังสือของ ohm)

43. **แก้บั๊กต่อ (ยังไม่ขึ้น) → เปลี่ยนแหล่งปกจาก Google Books เป็น Open Library (2026-09-14, วันเดียวกัน)** — ohm push ข้อ 42 แล้วบอก "ยังไม่ขึ้นนะ" กลับเข้าไปตรวจสดผ่าน Claude Browser บน network เดียวกับ ohm อีกรอบ (คนละ tab/browser กับ github.io app จริงเลย ไม่ใช่แค่ localStorage) ยืนยันว่า `preview-dashboard.html` บน GitHub Pages มีโค้ดข้อ 42 จริง (push สำเร็จแล้ว) แต่ยิง `fetch` ตรงไป Google Books API จาก network นี้ **ยังเจอ 429 อยู่** แม้พักไปนานกว่า 10-15 นาทีแล้ว — สรุปว่า Google Books แบบไม่มี API key มี **IP-level quota ที่ตึงเกินกว่าจะพึ่งได้จริงสำหรับ network ของ ohm** (ไม่ใช่แค่ burst ชั่วคราวจากตอนทดสอบแรกๆ)

    **เปลี่ยนไปใช้ Open Library แทนทั้งหมด** (`openlibrary.org/search.json` หา `cover_i` + `covers.openlibrary.org/b/id/{id}-M.jpg` โหลดรูป) — public API เหมือนกัน ไม่ต้องมี key เหมือนกัน แต่ทดสอบสดจาก network เดียวกันนี้ (ที่ Google Books ยัง 429 อยู่) ได้ผลทันที **status 200 ทั้ง 2 เล่ม** (Atomic Habits → cover_i 12539702, The Lean Startup → cover_i 7104760) ทดสอบโหลดรูปจริงจาก `covers.openlibrary.org` ก็ได้ `image/jpeg` กลับมาปกติ

    **โค้ดที่แก้:** เปลี่ยนเฉพาะเนื้อใน `fetchBookCover(title,author)` — endpoint/parsing เปลี่ยนจาก Google Books schema (`items[0].volumeInfo.imageLinks.thumbnail`) เป็น Open Library schema (`docs[0].cover_i` → ประกอบ URL รูปเอง) โครงสร้าง return `{status:'done'|'rate-limited'|'error',url}` และกลไก backoff/retry 24 ชม./เว้นจังหวะ 2.5 วิ/ปุ่มลองใหม่ด้วยมือจากข้อ 42 **ใช้ต่อได้ทั้งหมดโดยไม่ต้องแก้** (ออกแบบให้ไม่ผูกกับ provider ไว้แล้ว)

    **ทดสอบยืนยันก่อน push รอบนี้ (ต่างจากรอบก่อนที่ยังไม่ได้ยืนยัน):** รัน `fetchBookCover` เวอร์ชันใหม่ตรงๆ ในคอนโซลของเบราว์เซอร์จริง ohm ได้ผลลัพธ์ `{status:'done',url:'https://covers.openlibrary.org/b/id/...'}` ครบทั้ง 2 เล่มจริง ก่อนค่อย commit — มั่นใจกว่าข้อ 42 ที่ยัง push ไปทั้งที่ quota ไม่คืน

    **smoke-test.js:** ไม่ต้องแก้ (เหตุผลเดียวกับข้อ 42) — รันซ้ำ **ผ่าน 163/163 · NO RUNTIME ERRORS**

    **ขั้นต่อไป:** รอ ohm push แล้วเปิดแอปจริงเช็คว่าปกขึ้นจริง (รอบนี้ควรขึ้นทันทีไม่ต้องรอ backoff เพราะ Open Library ไม่ได้ติด 429 อยู่แล้ว) ถ้าขึ้นแล้วค่อยกลับไปถามลำดับงานต่อ (รีวิว/เรตติ้ง หรือ video/podcast หรือไอเดียเสริม — ค้างจากข้อ 41)

44. **เพิ่มอัปโหลดปกเอง (ทางเลือกผสมกับดึงอัตโนมัติ) (2026-09-14, ต่อจากข้อ 43 วันเดียวกัน)** — ข้อ 43 push แล้ว ohm เปิดแอปจริงเจอ **Atomic Habits ขึ้นปกจริงแล้ว** (ยืนยันว่า Open Library ใช้ได้จริง) แต่เล่ม "ถ้าอีก 1 ปีจะต้องตาย..." (หนังสือแปลไทยจากญี่ปุ่น ผู้แต่ง Ozawa Taketoshi) ไม่มีปก — **ไม่ใช่บั๊ก แต่เป็นข้อจำกัดของฐานข้อมูล Open Library ที่ครอบคลุมหนังสือภาษาอังกฤษเป็นหลัก แทบไม่มีหนังสือแปลไทย/ญี่ปุ่น** ohm ถามว่าควรอัปโหลดปกเองทั้งหมดไหม + ถามข้อเสียของการดึง API — ตอบไปว่า API มีข้อเสีย 3 อย่างหลัก (ครอบคลุมไม่ครบโดยเฉพาะหนังสือแปล/สำนักพิมพ์เล็ก, พึ่งบริการภายนอกตลอด, เสี่ยงได้ปกผิดเล่มถ้าชื่อ/ผู้แต่งตรงกับเล่มอื่นในฐานข้อมูล) แนะนำให้ **ทำผสมกัน** (ดึงอัตโนมัติเป็นค่าเริ่มต้น + เพิ่มปุ่มอัปโหลดเองไว้ใช้เฉพาะเล่มที่มีปัญหา) แทนการเลิกใช้ API ไปเลย — ohm เลือกตัวเลือกนี้ผ่าน `AskUserQuestion`

    **`BookFormModal`:** เพิ่มฟิลด์ `coverUrl`/`coverManual` ใน `blankForm` + เพิ่มส่วน "ปกหนังสือ" ในฟอร์ม (preview รูปเล็ก 52×78 + ปุ่ม "อัปโหลดปกเอง") — เลือกไฟล์แล้วย่อรูปด้วย `<canvas>` ในเบราว์เซอร์ก่อนเก็บเสมอ (กว้างสุด 300px คงสัดส่วนเดิม, ส่งออกเป็น JPEG q.82) แล้วเก็บเป็น **data URI ฝังตรงใน `coverUrl`** — ไม่ต้องสร้างระบบอัปโหลดไฟล์แยกเลย ใช้ field เดียวกับที่ auto-fetch ใช้อยู่แล้ว ซิงก์ผ่าน Drive JSON เดิมได้ทันที (เหมือนแนวทาง base64-embed ที่เคยคิดไว้ตอนแรกสุดก่อนเลือกไปทาง auto-fetch ก่อน — ตอนนี้ได้ใช้ทั้งคู่) เมื่ออัปโหลดแล้วจะโผล่ปุ่ม **"กลับไปดึงอัตโนมัติ"** (ล้าง `coverUrl`+`coverManual` ในฟอร์ม ให้ auto-fetch ลองใหม่ตอน save)

    **`BookQueuePage`:** `openEdit` ส่ง `coverUrl`/`coverManual` เข้าโมดัลด้วย · `saveBook` เขียน `coverUrl`/`coverManual` ลง record — ถ้ากด "กลับไปดึงอัตโนมัติ" (coverManual เป็น false และ coverUrl ว่าง) จะล้าง `coverCheckedAt` ด้วย ให้ auto-fetch ไล่เช็คใหม่ทันทีตอนเปิดหน้าครั้งหน้า · `needsCover()` เพิ่มเงื่อนไข `!b.coverManual` กันไม่ให้ auto-fetch loop ไปทับเล่มที่อัปโหลดเองไว้ · `retryCover()` (ปุ่มกดที่ไอคอนสำรอง) เพิ่ม guard ข้ามเล่ม `coverManual` เหมือนกัน

    **CSS:** เพิ่ม `.book-cover-picker`/`.book-cover-picker-preview`/`.book-cover-picker-actions` (ต่อท้ายบล็อกข้อ 24/9b/9c ใน `styles.css`)

    **smoke-test.js:** เพิ่ม 3 เช็คในบล็อกข้อ 24/9 เดิม (เปิดแก้ไขเล่มเดิม → ใส่ไฟล์รูปเทส (PNG 1×1 เข้ารหัส base64 inline ในเทสต์ ไม่ต้องมีไฟล์แยก) → เช็ค preview ขึ้นจริง → เช็คปุ่ม "กลับไปดึงอัตโนมัติ" โผล่มา → กด save → เช็ค `localStorage` มี `coverManual:true`+`coverUrl` เป็น data URI จริง) — **ผลรัน: ผ่าน 166/166 · NO RUNTIME ERRORS**

    **ขั้นต่อไป:** รอ ohm push แล้วลองอัปโหลดปกเองกับเล่มที่ดึงอัตโนมัติไม่เจอ (เช่นเล่มญี่ปุ่นที่ค้างอยู่ตอนนี้) ยืนยันว่าใช้งานได้จริงบนเครื่อง แล้วค่อยกลับไปคุยเรื่องรีวิว/เรตติ้ง หรือ video/podcast หรือไอเดียเสริมที่ค้างจากข้อ 41

45. **หน้ารีวิว+ให้คะแนน Book/Video/Podcast รวมรายการเดียว (2026-09-14, แทรกเป็นงานถัดไปก่อนคิวเดิมตามที่ ohm ขอ)** — สเปกคุยเคลียร์ล่วงหน้าไว้ในโปรเจกต์ claude.ai "secretory" ไฟล์ `spec-review-page.md`: **1 หน้า รายการเดียว ไม่แยก 3 section** — ใช้ field `type`:`book`/`video`/`podcast` คัดแยกทีหลังได้, คะแนนสเกล **1-10**, สโคปรอบนี้ทำแค่หน้ารีวิว+ให้คะแนนก่อน (ตัวกรอง/แท็ก, สถิติการอ่าน, คำคม **ยังไม่ทำ** เก็บไว้รอบหน้าตามที่ตัดสินใจไว้แล้ว) · session นี้ `device_bash` เสียอยู่ (Windows update 8 ก.ย. บล็อกการ mount) เลยใช้วิธี stage ไฟล์เข้าคอนเทนเนอร์ + commit กลับแทน

    **โครงสร้างข้อมูล:** เพิ่ม `data.mediaReviews[]` ที่ **root** (เหมือน `bookQueue`/`journal` ไม่ได้ซ้อนใน `finance`) แต่ละแถว `{id,title,type,rating(1-10),reviewText,date,link}` · เพิ่ม default ใน `DEFAULT_DATA` (มีตัวอย่าง 2 รายการไว้โชว์ตอนไม่มีข้อมูล) + merge fallback ใน `useSecretaryData` (`mediaReviews:parsed.mediaReviews||DEFAULT_DATA.mediaReviews`) กันข้อมูลเก่าที่ยังไม่มี field นี้พัง

    **หน้าใหม่ `ReviewsPage` + `ReviewFormModal`:** เขียนตามแพทเทิร์นเดียวกับ `BookQueuePage`/`BookFormModal` ทุกอย่าง — ฮีโร่สเกล 38px ตามธรรมนูญข้อ 32, ตัวเลขใหญ่ 3 ช่อง (หนังสือ/วิดีโอ/พอดแคสต์), **ปุ่มเพิ่มข้อมูลปุ่มเดียว** (ข้อ 32/2) เปิด modal เดียวกันทั้งเพิ่ม/แก้ไข, รายการทั้งหมดอยู่ใน `DashCollapse` กดกาง/ย่อได้ (ข้อ 28) เรียงวันที่ใหม่สุดก่อน · ฟอร์มมี title*, type (select), rating (range 1-10 โชว์ "x/10"), reviewText (textarea), date, link (ไม่บังคับ — ถ้ามีโผล่ปุ่มเปิดลิงก์ในแถว) · แถวแสดงจุดสีตามประเภท (`--blue`=หนังสือ, `--orange`=วิดีโอ, `--gold`=พอดแคสต์)

    **บั๊กที่เจอระหว่างทำและแก้แล้ว:**
    1. ตอนแรกตั้งชื่อปุ่มเมนูว่า **"Reviews"** — พอรัน smoke test แท็บ "Review" ของ Finance หายไป (0 ตัวอักษร) แล้วเทสต์ค้าง timeout รอปุ่ม "Overview" ตลอดกาล ตรวจสอบเจอว่า Playwright `has-text` จับซับสตริง ปุ่มเมนู "Reviews" (ซึ่งอยู่ใน DOM ก่อนแท็บ Finance) ไปโดน `button:has-text("Review")` จับคู่แทนปุ่มแท็บจริง ทำให้กดพลาดหน้าไปเปิดหน้ารีวิวแทนสลับแท็บ Finance → **แก้โดยเปลี่ยนชื่อปุ่มเมนูเป็น "Media"** แทน (หัวข้อในหน้ายังใช้ "Reviews" ได้ปกติเพราะไม่ใช่ปุ่ม) ยืนยัน rerun ผ่านแล้ว
    2. ตอนแรกใช้ `var(--pink)` เป็นสีป้ายพอดแคสต์ — smoke test จับได้ว่าเป็นสีกลุ่มม่วงเก่า (ตรงกับ `LEGACY_COLOR` migration map ที่มี `#f472b6`→`#d9b26a` อยู่แล้ว แปลว่าเป็นสีที่แอปตั้งใจเลิกใช้ตั้งแต่รีดีไซน์ข้อ 23) → เปลี่ยนเป็น `var(--gold)` แทน

    **smoke-test.js:** เพิ่ม seed `mediaReviews` 1 รายการ + บล็อกเทสต์ใหม่ 8 เช็ค (ฮีโร่/bignum/ปุ่มเดียว/รายการจาก seed, เปิดโมดัลเพิ่ม+บันทึกจริง, เปิดโมดัลแก้ไขพร้อมค่าเดิม, ลบแล้วหายจริงจาก localStorage) + เพิ่ม "Media" เข้าไปในลูปเช็คสีม่วง/สเกลข้อ 24/6 — **ผลรันจริงก่อน commit: ผ่าน 176/176 · NO RUNTIME ERRORS** (baseline ไฟล์เดิมก่อนแก้ก็ผ่าน 166/166 ครบ ยืนยันว่าของเดิมไม่พัง เพิ่มมาแค่ 10 เช็คของฟีเจอร์ใหม่)

    **ขั้นต่อไป:** รอ ohm push แล้วลองใช้งานจริง (เพิ่ม/แก้ไข/ลบรีวิวหนังสือ/วิดีโอ/พอดแคสต์จากเมนู "Media") ยืนยันว่าตรงกับที่ต้องการ แล้วค่อยกลับไปคุยไอเดียเสริม (ตัวกรอง/แท็ก, สถิติการอ่าน, คำคม) หรือกลับไปทำงานค้างเดิมที่ถูกแทรกคิวไว้ (XIRR/ประวัติ DCA/เป้าสัดส่วนพอร์ต/กราฟมูลค่าพอร์ต, Tracker Timeline/Review รายเดือน) · **ohm ยังต้อง push เอง**

46. **ทบทวนข้อ 45 ใหม่ตามฟีดแบ็ก ohm — เลิกเมนูบนแยก ย้ายเป็นแท็บย่อยในหน้า Books แทน (2026-09-14, วันเดียวกัน ก่อน ohm push ข้อ 45 ด้วยซ้ำ)** — ohm บอกว่าเมนูบนเยอะเกินไปแล้ว ไม่อยากเพิ่มอีกอัน ขอให้ย้ายไปเป็นแท็บย่อยในหน้า Books แทน + ขอให้แยกรีวิวหนังสือออกจากรีวิว video/podcast (รีวิวหนังสือให้ติดอยู่กับ record หนังสือเล่มนั้นเลย ไม่ต้องรวมหน้าเดียวกัน) + ขอชื่อแท็บที่ไม่ใช่ "Media" แบบคูลๆ — ถามกลับไปด้วย `AskUserQuestion` เสนอ 4 ชื่อ (Playback/Replay/On Air/Screen & Sound) **ohm เลือก "Playback"**

    **โครงสร้างใหม่ (แทนที่ข้อ 45 เดิมทั้งหมด ไม่ใช่ทำเพิ่ม):**
    - **ลบเมนูบน "Media" ออกทั้งหมด** — `PAGE_TITLES`/`PageNav`/routing/ผลค้นหา กลับไปเป็น 7 หน้าเดิม (Home/Finance/Tracker/Health/Documents/Books/Journal)
    - **รีวิวหนังสือย้ายไปติดกับ record หนังสือใน `bookQueue` ตรงๆ** — เพิ่ม field `rating`(1-10 ไม่บังคับ)/`reviewText` ใน `BookFormModal` (ช่องกรอกใหม่ต่อจากความสำคัญ) + โชว์คะแนนบนการ์ดชั้นหนังสือ (เช่น "★ 9/10" ต่อท้ายชื่อผู้เขียน)
    - **หน้า Books มี 2 แท็บย่อย** (แพทเทิร์นเดียวกับ `FinancePage` — `.fin-tab-nav`/`.fin-tab-btn`) ผ่าน component ใหม่ `BooksPage`: แท็บ "Book Queue" (เนื้อหาเดิมทั้งหมด ย้ายจาก `BookQueuePage`→เปลี่ยนชื่อเป็น `BookQueueTab` โค้ดข้างในไม่เปลี่ยน) กับแท็บ **"Playback"** (`PlaybackTab`/`PlaybackFormModal` — เดิมชื่อ `ReviewsPage`/`ReviewFormModal` แต่ตัดตัวเลือกประเภท "หนังสือ" ออก เหลือแค่วิดีโอ/พอดแคสต์ 2 ประเภท, bignums เหลือ 2 ช่องไม่ใช่ 3)
    - `data.mediaReviews[]` ยังใช้ชื่อเดิม (มีความหมายตรงยังคือ "วิดีโอ+พอดแคสต์" ก็ยังนับเป็น media ได้) แต่ตอนนี้เก็บแค่ `type:video`/`podcast` เท่านั้น ไม่มี `book` อีกต่อไป

    **smoke-test.js:** อัปเดต seed (`bookQueue` bk2 ใส่ `rating`/`reviewText` ตัวอย่าง, `mediaReviews` เปลี่ยนจาก book→video), ลบบล็อกเทสต์เมนู "Media" เดิมทั้งหมด แทนที่ด้วย (1) เช็คว่าคะแนน/รีวิวที่ติดกับหนังสือ (seed bk2) โชว์บนชั้นหนังสือ+แก้ไข/บันทึกได้จริง และ (2) บล็อกเทสต์แท็บ "Playback" ใหม่ (คลิก `.fin-tab-btn:has-text("Playback")` แทนปุ่มเมนูบน, เช็ค select ตัวเลือกประเภทเหลือแค่ 2 ตัว, ปุ่มเดียวเพิ่ม/แก้ไข/ลบเหมือนเดิม) — **ผลรันจริงก่อน commit: ผ่าน 178/178 · NO RUNTIME ERRORS**

    **ขั้นต่อไป:** รอ ohm push แล้วลองใช้งานจริง — เปิดหน้า Books เช็คว่าเห็น 2 แท็บย่อย (Book Queue/Playback), ลองให้คะแนน+รีวิวหนังสือเล่มหนึ่งจากปุ่มแก้ไข, ลองเพิ่มรีวิววิดีโอ/พอดแคสต์จากแท็บ Playback ยืนยันว่าตรงใจ แล้วค่อยกลับไปคุยไอเดียเสริม หรืองานค้างเดิม (XIRR ฯลฯ, Tracker Timeline) · **ohm ยังต้อง push เอง**

47. **เพิ่มรูปปกให้วิดีโอ/พอดแคสต์ในแท็บ Playback (2026-09-14, วันเดียวกัน ก่อน ohm push ข้อ 46 ด้วยซ้ำ)** — ohm ขอให้เพิ่มรูปปกได้เหมือนหนังสือ

    **ตัดสินใจ:** ไม่มี auto-fetch แบบหนังสือ (ไม่มี API ฟรีที่เชื่อถือได้สำหรับ YouTube/พอดแคสต์แบบไม่ต้องขอ key) — ใช้ **อัปโหลดเองอย่างเดียว** เอาโค้ดย่อรูปด้วย `<canvas>` จาก `BookFormModal` (กว้างสุด 300px, JPEG q.82, เก็บเป็น data URI) มาใช้ซ้ำใน `PlaybackFormModal` ตรงๆ — ต่างจากหนังสือตรงที่ไม่ต้องมี field `coverManual`/ปุ่ม "กลับไปดึงอัตโนมัติ" เพราะไม่มีระบบอัตโนมัติให้สลับกลับ (มีแค่ "อัปโหลดปก"/"ลบปก")

    **`PlaybackFormModal`:** เพิ่ม field `coverUrl` ใน `blankForm` + ส่วน "ปก (ไม่บังคับ)" ในฟอร์ม (preview 64×64 สี่เหลี่ยมจัตุรัส — ต่างจากปกหนังสือที่เป็นทรง 2:3 แนวตั้ง — ใช้คลาสใหม่ `.pb-cover-picker-preview` ซ้อนทับขนาดจาก `.book-cover-picker-preview` เดิม) ไอคอน fallback ตอนยังไม่มีปกเปลี่ยนตามประเภท (`VideoIcon`/`PodcastIcon` เพิ่มใหม่)

    **`PlaybackTab`:** แถวรายการในลิสต์เพิ่ม thumbnail เล็ก 40×40 (`.pb-thumb`) หน้าชื่อเรื่อง โชว์รูปที่อัปโหลดหรือไอคอน fallback ตามประเภทถ้ายังไม่มี · `openEdit`/`saveReview` ส่ง/เก็บ `coverUrl` ด้วย

    **CSS:** เพิ่ม `.pb-cover-picker-preview`/`.pb-row`/`.pb-thumb` ต่อท้ายบล็อก `.book-cover-picker` เดิมใน `styles.css`

    **smoke-test.js:** เพิ่ม 3 เช็คใหม่ในบล็อก Playback (อัปโหลดรูปเทสแล้วขึ้น preview จริง, บันทึก `coverUrl` เป็น data URI ลง `localStorage` จริง, thumbnail โชว์ในลิสต์จริง) — **ผลรันจริงก่อน commit: ผ่าน 181/181 · NO RUNTIME ERRORS**

    **ขั้นต่อไป:** รอ ohm push แล้วลองอัปโหลดปกวิดีโอ/พอดแคสต์จริงดูว่าใช้งานลื่นไหม · **ohm ยังต้อง push เอง**

48. **Playback เป็นชั้นวาง (shelf) + จัดกลุ่มเป็นการ์ดโฟลเดอร์ตามช่อง/เพลย์ลิสต์ (2026-09-16)** — ohm อยากให้ Playback ทำตัวเหมือน shelf หนังสือ และรวมรายการจากช่อง/เพลย์ลิสต์เดียวกันเป็นกลุ่มได้ — ถามกลับด้วย `AskUserQuestion` 3 ข้อก่อนลงมือ (ohm เลือก recommended ทั้ง 3 ข้อ):

    **ตัดสินใจที่ยืนยันแล้ว (ห้ามถามซ้ำ):**
    - **กลไกเก็บข้อมูล:** field ข้อความล้วน `channel` (ไม่บังคับ) บนแต่ละรายการ — เหมือน "ผู้แต่ง" ของหนังสือ ไม่ใช่เอนทิตี playlist แยก รายการที่ชื่อ `channel` ตรงกันเป๊ะจะถูกจัดกลุ่มอัตโนมัติตอน render (ไม่มี normalize ตัวพิมพ์/ช่องว่าง — ใส่ `<datalist>` autocomplete จากชื่อช่องที่เคยกรอกไว้กันพิมพ์เพี้ยน)
    - **เปิดโฟลเดอร์:** ขยายลงมาในหน้าเดิม (ไม่เปิด modal) — กดการ์ดโฟลเดอร์ → แผง `.pb-folder-panel` โผล่ต่อท้ายชั้นวาง (ไม่ใช่ inline ใต้การ์ดตัวเองใน grid เพราะจะพังเลย์เอาต์ grid) แสดงรายการทุกตอนในช่องนั้นแบบ `.db-row` เดิม (แก้ไข/ลบ/เปิดลิงก์ได้ครบ) — เปิดได้พร้อมกันหลายช่อง (state `openFolders` เป็น object ไม่ใช่ toggle เดี่ยว)
    - **ปกการ์ดโฟลเดอร์:** ใช้ปกของตอนล่าสุดในกลุ่มอัตโนมัติ (ไม่มีระบบตั้งปกแยกของโฟลเดอร์เอง) — badge ตัวเลขจำนวนตอนที่มุมปก (`.pb-folder-badge`)
    - **ชื่อฟิลด์ในฟอร์ม:** "ช่อง/เพลย์ลิสต์"

    **`PlaybackFormModal`:** เพิ่ม field `channel` ใน `blankForm` + input พร้อม `<datalist id="pb-channel-options">` (รับ prop `channelOptions` จาก `PlaybackTab`) วางไว้ใต้ชื่อเรื่อง เหนือช่องปก — **แก้บั๊กระหว่างพัฒนา:** เดิมใช้ `initial?"แก้ไข":"เพิ่ม"` ตัดสินหัวข้อ/ปุ่ม ซึ่งพังทันทีที่เริ่มส่ง `initial` แบบ partial ตอน add (prefill ช่อง) เพราะ `initial` truthy แต่ไม่ใช่โหมดแก้ไข → เปลี่ยนไปรับ prop `mode` ตรงๆ ตัดสินด้วย `mode==="edit"` แทน

    **`PlaybackTab`:** คำนวณ `channelOptions` (unique sorted จาก `reviews`), เก็บ `channel` ใน `saveReview`/`openEdit`, จัดกลุ่ม `sorted` reviews เป็น `standalone[]` (ไม่มี channel) + `groupsMap` (channel→items) แล้วรวมเป็น `shelfEntries` เดียวเรียงตามวันที่ล่าสุดของแต่ละใบ (การ์ดเดี่ยวใช้วันที่ตัวเอง, การ์ดโฟลเดอร์ใช้วันที่ตอนล่าสุดในกลุ่ม) — เปลี่ยน UI list เดิม (flat `db-row` ทั้งหมด) เป็นชั้นวาง `.pb-shelf` (grid การ์ด เหมือน `.book-shelf` ของ Book Queue) ปกสี่เหลี่ยมจัตุรัส 1:1 (ต่างจากหนังสือที่เป็น 2:3) การ์ดโฟลเดอร์กดได้ (`.pb-folder`, cursor pointer) มี badge จำนวนตอน + เฉลี่ยคะแนน · แผง `.pb-folder-panel` ที่ขยายมีปุ่ม "เพิ่มตอนในช่องนี้" (`openAdd(channel)` prefill ช่องให้อัตโนมัติ)

    **บั๊กที่เจอจาก smoke-test ระหว่างพัฒนา (แก้แล้ว):** ปุ่ม "เพิ่มข้อมูล" หลักเดิม `onClick={openAdd}` ส่ง React SyntheticEvent เข้าไปตรงๆ — พอ `openAdd` เปลี่ยนมารับพารามิเตอร์ `prefillChannel` ทำให้ event object หลุดเข้าไปเป็นค่า `channel` (`String(event)` = `"[object Object]"`) เจอจากเช็ค "รายการที่ไม่มีช่อง...ยังเป็นการ์ดเดี่ยว" fail แบบงงๆ ตอนดีบัก — แก้เป็น `onClick={()=>openAdd()}`

    **CSS:** เพิ่ม `.pb-shelf/.pb-shelf-item/.pb-shelf-cover/.pb-shelf-info/.pb-shelf-actions/.pb-folder/.pb-folder-badge/.pb-folder-panel` ต่อท้ายบล็อก `.pb-thumb` เดิมใน `styles.css` + breakpoint มือถือ `.pb-shelf{grid-template-columns:repeat(auto-fill,minmax(100px,1fr))}` เหมือน `.book-shelf`

    **DEFAULT_DATA:** seed `mediaReviews` เพิ่มตัวอย่างช่องเดียวกัน 2 ตอน (mr1/mr3 = "Huberman Lab") ให้เห็นการจัดกลุ่มทันทีตอนเปิดแอปครั้งแรก

    **smoke-test.js:** เพิ่ม 6 เช็คใหม่ในบล็อกใหม่ "[ข้อ 48]" (เพิ่ม 2 ตอนชื่อช่องตรงกัน→รวมเป็นการ์ดเดียว, badge จำนวนถูกต้อง, กดการ์ดแล้วขยายแสดงครบ 2 แถว, ปุ่ม "เพิ่มตอนในช่องนี้" prefill ช่องถูก, รายการไม่มีช่องยังเป็นการ์ดเดี่ยว) + แก้ 2 เช็คเดิมที่อ้าง class `.db-row`/`.pb-thumb` ของลิสต์แบบเก่าให้ตรงกับโครง `.pb-shelf-item`/`.pb-shelf-cover` ใหม่ — **ผลรันจริงก่อน commit: ผ่าน 187/187 · NO RUNTIME ERRORS**

    **ขั้นต่อไป:** รอ ohm push แล้วลองจัดกลุ่มช่อง/เพลย์ลิสต์จริงดูว่าใช้งานลื่นไหม (โดยเฉพาะกรณีพิมพ์ชื่อช่องเพี้ยนแล้วไม่รวมกลุ่ม — ยังไม่มี normalize) · **ohm ยังต้อง push เอง**

49. **ช่อง/เพลย์ลิสต์เปลี่ยนจากพิมพ์อิสระเป็น dropdown บังคับเลือก (2026-09-16, วันเดียวกัน ก่อน push ข้อ 48 ด้วยซ้ำ)** — ต่อจากข้อ 48 ที่ปิดท้ายไว้ว่ายังไม่มี normalize ชื่อช่อง ohm เลือกตัวเลือกนี้จากเมนู "มีงานอะไรแนะนำต่อ" ที่เสนอไป (อีก 3 ตัวเลือกที่ไม่ได้เลือก: Investments เพิ่มเติม/Tracker เพิ่มเติม/Book-Playback ไอเดียเสริม — ยังค้างอยู่เหมือนเดิม)

    **แนวทาง:** ไม่เปลี่ยน data model (ยังเป็น field `channel` ข้อความล้วนบน `mediaReviews` เหมือนข้อ 48) เปลี่ยนแค่ UI ใน `PlaybackFormModal` จาก `<input list="pb-channel-options">` (พิมพ์อิสระ+datalist ช่วยเสนอ แต่ยังพิมพ์เพี้ยนได้อยู่) เป็น `<select id="pb-channel-select">` บังคับเลือกจากชื่อที่มีอยู่แล้ว (`channelOptions` เดิม) + ตัวเลือกพิเศษ `"__new__"` = "+ สร้างช่องใหม่..." ที่สลับ UI ไปเป็นช่องพิมพ์ข้อความชั่วคราว (state ใหม่ `newChannelMode`) มีปุ่ม "ยกเลิก" กลับไป dropdown + เคลียร์ค่า — ค่า `form.channel` ที่ส่งไป `saveReview` ยังเป็น string เดิมทุกกรณี ไม่กระทบโครงสร้างข้อมูล/การจัดกลุ่มใน `PlaybackTab` เลย

    **id ใหม่ที่เพิ่ม:** `#pb-channel-select` (dropdown ช่อง) และ `#pb-type-select` (dropdown ประเภท วิดีโอ/พอดแคสต์ — เดิมไม่มี id ทำให้ query `.modal-backdrop select option` ตอนเทสต์ดึงมาปนกับ dropdown ช่องใหม่ที่เพิ่งเพิ่ม ต้องใส่ id แยกให้ query เจาะจงได้)
